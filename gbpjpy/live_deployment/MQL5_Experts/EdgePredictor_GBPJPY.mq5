//+------------------------------------------------------------------+
//|  EdgePredictor_GBPJPY.mq5                                       |
//|  GBPJPY M5 — Rule + ML-confirmation system                      |
//|                                                                   |
//|  Architecture:                                                    |
//|   1. Detect weekly regime -> cluster (C0/C1/C2/C3)              |
//|   2. For each cluster: scan all rules belonging to that cluster  |
//|      on the just-closed bar                                       |
//|   3. For each fired rule, run its ONNX confirmation classifier;  |
//|      take the trade only if probability >= rule's threshold      |
//|                                                                   |
//|   48 hand-coded setup rules (12+14+12+10 across 4 clusters)     |
//|   48 ONNX classifiers (1 per rule)                               |
//|   1  regime selector (constants in regime_selector_gbpjpy.mqh)   |
//|                                                                   |
//|   SL = SL_ATR_MULT x ATR(14)                                     |
//|   TP = TP_ATR_MULT x ATR(14)                                     |
//+------------------------------------------------------------------+
#property copyright "EdgePredictor"
#property version   "1.00"
#property description "AI-powered GBPJPY rule + ML confirmation trading system."
#property description "Requires license key. Allow WebRequest for your server URL in MT5 settings."
#property strict

#include <Trade/Trade.mqh>
#include <regime_selector_gbpjpy.mqh>
#include <confirmation_router_gbpjpy.mqh>
...
input int    InpMaxSpread          = 50;    // skip entry if spread > N points (GBPJPY wider spread)
input long   InpMagic              = 420307;
input int    InpMaxDevPoints       = 50;

input group "=== Debug ==="
input bool   InpVerbose            = true;

//--- constants  (MUST match labeler.py / build_setup_signals.py)
static const double TP_ATR_MULT    = 6.0;   // Samurai: widened from 2.0 → 6.0 (PF 1.35 → 1.73)
static const double SL_ATR_MULT    = 2.0;   // Samurai: widened from 1.0 → 2.0 (size /2 to keep $-risk)
static const int    FEATURE_DIM    = 34;
static const int    LOOKBACK       = 120;    // bars to copy for feature compute
...
void SetupChart()
{
   ...
   ChartSetInteger(chart, CHART_COLOR_FOREGROUND,     0x5A5A5A);  // axis labels
   ChartSetInteger(chart, CHART_COLOR_GRID,           0x1A1A1A);
   ChartSetInteger(chart, CHART_COLOR_CHART_UP,       0x10B981);  // bull candle
   ChartSetInteger(chart, CHART_COLOR_CHART_DOWN,     0xEF4444);  // bear candle
...
//+------------------------------------------------------------------+
//| On-chart dashboard (top-left comment box)                         |
//+------------------------------------------------------------------+
void UpdateDashboard()
{
   ...
   string dash = "";
   dash += "═══════════════════════════════════════\n";
   dash += "   EdgePredictor Samurai  v1.0\n";
   dash += "   GBPJPY · M5 · Rule + ML Confirmation\n";
   dash += "═══════════════════════════════════════\n";
   dash += "\n";
   ...
   dash += StringFormat("   ATR(14):    %.3f\n", RawATR14_Quick());
   dash += "═══════════════════════════════════════\n";

   Comment(dash);
}

double RawATR14_Quick()
{
   MqlRates r[];
   ArraySetAsSeries(r, true);
   if(CopyRates(_Symbol, MODEL_TF, 0, 15, r) < 15) return 0;
   double s = 0;
   for(int k = 0; k < 14; k++)
   {
...
//+------------------------------------------------------------------+
//| Count session trades & P&L from history                           |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Compute EMA(span) of close using EWM walk                        |
//+------------------------------------------------------------------+
double EWMClose(const MqlRates &rb[], int idx, int span)
{
...
//+------------------------------------------------------------------+
//| Compute RSI(14) using EWM (matches Python ewm(span=14))          |
//+------------------------------------------------------------------+
double EWMRSI14(const MqlRates &rb[], int idx)
{
   int maxbars = ArraySize(rb);
   int start = MathMin(idx + 100, maxbars - 1);
...
//+------------------------------------------------------------------+
//| Compute stoch_k: raw %K over 14, smoothed by 3-bar SMA           |
//+------------------------------------------------------------------+
double SmoothedStochK(const MqlRates &rb[], int idx)
{
   if(idx + 16 >= ArraySize(rb)) return 0.5;
   double sum = 0;
   for(int j = 0; j < 3; j++)
   {
      int bi = idx + j;
      double lo = rb[bi].low, hi = rb[bi].high;
      for(int k = bi; k < bi + 14 && k < ArraySize(rb); k++)
      {
         lo = MathMin(lo, rb[k].low);
         hi = MathMax(hi, rb[k].high);
      }
      sum += _sd(rb[bi].close - lo, hi - lo);
   }
   return sum / 3.0;
}

//+------------------------------------------------------------------+
//| Compute consecutive direction count                               |
//+------------------------------------------------------------------+
double ConsecDir(const MqlRates &rb[], int idx)
{
   double dir0 = _sg(rb[idx].close - rb[idx].open);
   if(dir0 == 0) return 0;
   double cnt = dir0;
   for(int k = idx + 1; k < ArraySize(rb); k++)
   {
      double dk = _sg(rb[k].close - rb[k].open);
      if(dk != dir0 || dk == 0) break;
      cnt += dir0;
   }
   return cnt;
}
...
//+------------------------------------------------------------------+
//| Compute vol_ratio: abs(delta_c) / rolling_20_mean(abs(delta_c))   |
//+------------------------------------------------------------------+
double VolRatio(const MqlRates &rb[], int idx)
{
   if(idx + 21 >= ArraySize(rb)) return 1.0;
   double vol0 = MathAbs(rb[idx].close - rb[idx+1].close);
   double sum = 0;
   for(int k = 0; k < 20; k++)
   {
      int bi = idx + k;
      if(bi + 1 >= ArraySize(rb)) break;
      sum += MathAbs(rb[bi].close - rb[bi+1].close);
   }
   double ma = sum / 20.0;
   if(ma < 1e-10) return 1.0;
   return vol0 / ma;
}

//+------------------------------------------------------------------+
//| Build 34-feature vector (21 base + 13 tech)                       |
//+------------------------------------------------------------------+
bool BuildFeatures(const MqlRates &rb[], float &feat[])
{
   ...
   // 13 tech features matching Python ALL_FEAT_COLS
   feat[21]=(float)SmoothedStochK(rb, i);                    // stoch_k
   feat[22]=(float)EWMRSI14(rb, i);                          // rsi14
   feat[23]=(float)BollingerPctB(rb, i, 20);                 // bb_pct
   feat[24]=(float)VolRatio(rb, i);                           // vol_ratio
   double bar_range_val = range;
   feat[25]=(float)_sd(bar_range_val, atr);                   // range_atr

   // dist_sma20 = (c - sma20) / atr
   double sma20 = 0;
   for(int k=0;k<20&&k<ArraySize(rb);k++) sma20 += rb[k].close;
   sma20 /= 20.0;
   feat[26]=(float)_sd(c - sma20, atr);                      // dist_sma20

   // dist_sma50 = (c - sma50) / atr
   double sma50 = 0;
   int cnt50 = MathMin(50, ArraySize(rb));
   for(int k=0;k<cnt50;k++) sma50 += rb[k].close;
   sma50 /= (double)cnt50;
   feat[27]=(float)_sd(c - sma50, atr);                      // dist_sma50

   feat[28]=(float)_sd(body, range);                          // body_ratio
   feat[29]=(float)ConsecDir(rb, i);                          // consec_dir
   ...
   feat[30]=(float)MathSin(2.0*M_PI*(double)dt.hour/24.0);   // hour_sin
   feat[31]=(float)MathCos(2.0*M_PI*(double)dt.hour/24.0);   // hour_cos
   feat[32]=(float)MathSin(2.0*M_PI*(double)py_dow/5.0);     // dow_sin (python-aligned)
   feat[33]=(float)MathCos(2.0*M_PI*(double)py_dow/5.0);     // dow_cos (python-aligned)
...
void RefreshActiveRegime()
{
   ...
   Print("GJ: active regime = C", g_active_cluster, " ", REGIME_NAMES[g_active_cluster],
         " (tradeable=", REGIME_TRADEABLE[g_active_cluster], ")");
}

//+------------------------------------------------------------------+
//| Scan all rules in active cluster, confirm with ONNX, return best |
//+------------------------------------------------------------------+
int ScanAndConfirm(const float &feat[], const MqlRates &rb[], int &out_rule)
{
   ...
   for(int r = 0; r < RULE_COUNT; r++)
   {
      ...
      int dir = CheckRule(r, feat, rb);
      if(dir == 0) continue;

      double prob = 0.0;
      if(!ConfirmRule(r, feat, prob)) continue;
...
int RefreshLicenseStatus(bool startup_check)
{
   ...
   {
      Print("EdgePredictor GBPJPY: No license key provided. Paste your key in Inputs tab.");
      if(startup_check)
         Alert("EdgePredictor GBPJPY\nLicense key missing\nPaste your license key in the Inputs tab.");
   ...
   if(res == 200)
   {
      ...
      if(startup_check)
         Print("EdgePredictor GBPJPY: License validated successfully.");
   ...
   if(res == 403)
   {
      ...
      g_licensed = false;
      Print("EdgePredictor GBPJPY: license inactive - ", message);
      if(startup_check)
         Alert("EdgePredictor GBPJPY\nLicense: ", reason, "\n", message);
      return LICENSE_STATUS_INVALID;
   }
   ...
   g_licensed = false;
   PrintFormat("EdgePredictor GBPJPY: license check failed (HTTP %d, err=%d)", res, GetLastError());
   if(startup_check)
...
bool ValidateLicense()
{
   ...
   {
      Comment("EdgePredictor GBPJPY v1.0\n\n  License: ACTIVE\n  Status: Ready to trade");
      return true;
   }
   return false;
}
...
bool DownloadModel(string filename)
{
   ...
   if(res != 200)
   {
      PrintFormat("EdgePredictor GBPJPY: model download HTTP %d for %s (err=%d)", res, filename, GetLastError());
      return false;
   }
   int fh = FileOpen(filename, FILE_WRITE | FILE_BIN);
   if(fh == INVALID_HANDLE)
   {
      PrintFormat("EdgePredictor GBPJPY: cannot write '%s' (err=%d)", filename, GetLastError());
      return false;
   }
   FileWriteArray(fh, result, 0, ArraySize(result));
   FileClose(fh);
   PrintFormat("EdgePredictor GBPJPY: '%s' downloaded (%d bytes)", filename, ArraySize(result));
   return true;
}
...
bool DownloadAllModels()
{
   ...
   for(int r = 0; r < RULE_COUNT; r++)
   {
      ...
      {
         PrintFormat("EdgePredictor GBPJPY: FAILED to download %s - aborting", fname);
         return false;
      }
      ok++;
      Comment(StringFormat("EdgePredictor GBPJPY v1.0\n\n  License: ACTIVE\n  Downloading models... %d/%d", ok, total));
   }
   // Vol-regime sizer — non-fatal: falls back to flat 1.0x lots if unavailable
   if(DownloadModel("regime_v4_gj.onnx"))
   {
      ok++;
      Comment(StringFormat("EdgePredictor GBPJPY v1.0\n\n  License: ACTIVE\n  Downloading models... %d/%d", ok, total));
   }
   ...
   {
      Print("EdgePredictor GBPJPY: regime_v4_gj.onnx unavailable on server — continuing without vol sizing");
   }
   PrintFormat("EdgePredictor GBPJPY: %d of %d models downloaded from server", ok, total);
   return true;
}
...
int OnInit()
{
   // Auto-detect GBPJPY variant
   if(StringFind(_Symbol, "GBPJPY") < 0 && StringFind(_Symbol, "gbpjpy") < 0)
   {
      Alert("EdgePredictor Samurai\nPlease attach to a GBPJPY chart.");
      return INIT_FAILED;
   }

   // Professional chart styling + auto-switch to M5
   SetupChart();
   DrawWatermark();

   Comment("EdgePredictor Samurai v1.0\n\n  Validating license...");
   ...
   {
      Print("EdgePredictor Samurai: failed to load confirmation models");
      return INIT_FAILED;
   }
   // Vol-regime sizer (regime_v4) — non-fatal if it fails; falls back to 1.0× lots
   if(!RegimeV4_Load("regime_v4_gj.onnx"))
      Print("EdgePredictor Samurai: regime_v4 sizer unavailable — using flat 1.0× lots");
   ...
   PrintFormat("EdgePredictor Samurai ready | regime=C%d %s | refresh=%d bars long=%s short=%s",
               g_active_cluster, REGIME_NAMES[g_active_cluster],
...
//+------------------------------------------------------------------+
//| Draw watermark on chart                                           |
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//| Report stats to server for admin dashboard                        |
//+------------------------------------------------------------------+
void ReportStats()
{
   double wr = (g_total_trades > 0) ? 100.0 * g_wins / g_total_trades : 0;
   ...
   string json = StringFormat(
      "{\"ea_name\":\"Samurai\",\"account\":\"%s\",\"symbol\":\"%s\","
   ...
   if(res == 200)
      Print("Stats reported to server");
   else if(InpVerbose)
      PrintFormat("Stats report HTTP %d (err=%d)", res, GetLastError());
}
...
void DrawWatermark()
{
   ...
   ObjectSetString (chart, name, OBJPROP_TEXT,       "EdgePredictor Samurai  |  Jay  |  edgepredictor.pro");
...
void OnTick()
{
   ...
   // Skip non-tradeable cluster
   if(REGIME_TRADEABLE[g_active_cluster] == 0)
   {
      if(InpVerbose)
         PrintFormat("GJ %s | regime=C%d %s | SKIP (non-tradeable)",
   ...
   {
      if(InpVerbose) Print("GJ: CopyRates insufficient");
      return;
   }
   ...
   {
      if(InpVerbose)
         PrintFormat("GJ: spread %d > %d, skipping", (int)spread_pts, InpMaxSpread);
      return;
   }
   ...
   {
      if(dir == 0)
         PrintFormat("GJ %s | regime=C%d %s | no signal",
                     TimeToString(bt, TIME_DATE|TIME_MINUTES),
                     g_active_cluster, REGIME_NAMES[g_active_cluster]);
      else
         PrintFormat("GJ %s | regime=C%d %s | rule=%s dir=%+d",
                     TimeToString(bt, TIME_DATE|TIME_MINUTES),
                     g_active_cluster, REGIME_NAMES[g_active_cluster],
   ...
   // Vol-regime DETECTION only (for logging). Sizing multiplier disabled for
   // Samurai — backtest showed HighVol 1.3x hurt PF on GBPJPY (1.73 → 1.64).
   // Midas benefits from sizing; Samurai works best with flat lots.
   double regime_prob = 0.0;
   int vol_regime = RegimeV4_Predict(regime_prob);
   double lots = NormLots(InpLots);  // flat — no sizing
   if(InpVerbose && vol_regime >= 0)
      PrintFormat("GJ: vol=%s (p=%.2f)  lots=%.2f  (vol sizing disabled on GJ)",
                  REGIME_V4_NAMES[vol_regime], regime_prob, lots);
   ...
   if(dir == +1)
   {
      ...
      ok = g_trade.Buy(lots, _Symbol, tick.ask, sl, tp,
                       StringFormat("GJ-L %s", RULE_NAMES[rule_id]));
   }
   else
   {
      ...
      ok = g_trade.Sell(lots, _Symbol, tick.bid, sl, tp,
                        StringFormat("GJ-S %s", RULE_NAMES[rule_id]));
   }
   if(!ok && InpVerbose)
      PrintFormat("GJ: order failed (%d)", GetLastError());
}
//+------------------------------------------------------------------+
