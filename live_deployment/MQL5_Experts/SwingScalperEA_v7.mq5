...
#include <confirmation_router_v7.mqh>
...
input long   InpMagic              = 420705;   // v7 — distinct from v6 (420305) so reporter can filter
...
static const double META_THRESHOLD = 0.675;  // v7 meta-labeling threshold (López de Prado)
...
bool ExitModel_Load()
{
   g_exit_handle = OnnxCreate("exit_v7.onnx", ONNX_DEFAULT);
   if(g_exit_handle == INVALID_HANDLE)
   { Print("ExitModel: failed to load exit_v7.onnx err=", GetLastError()); return false; }
   ...
   Print("ExitModel: exit_v7.onnx loaded"); return true;
}
...
bool MetaModel_Load()
{
   g_meta_handle = OnnxCreate("meta_v7.onnx", ONNX_DEFAULT);
   if(g_meta_handle == INVALID_HANDLE)
   { Print("MetaModel: failed to load meta_v7.onnx err=", GetLastError()); return false; }
   ...
   { Print("MetaModel: shape fail"); return false; }
   Print("MetaModel: meta_v7.onnx loaded (threshold=", META_THRESHOLD, ")");
   return true;
}
...
bool MetaModel_Accept(const float &feat_v7[], int direction, int cid, double &out_pwin)
{
   ...
   { Print("MetaModel: OnnxRun failed"); return false; }
...
int ScanAndConfirm(const float &feat_rules[], const float &feat_v7[], const MqlRates &rb[], int &out_rule)
{
   ...
   for(int r = 0; r < RULE_COUNT; r++)
   {
      ...
      double prob = 0.0;
      bool pass = ConfirmV7_Rule(r, feat_v7, prob);
      if(!pass) continue;
...
bool DownloadAllModels()
{
   ...
   for(int r = 0; r < RULE_COUNT; r++)
   {
      if(RULE_DISABLED_V7[r]) { ok++; continue; }              // skip disabled rules
      string fname = RULE_ONNX_V7[r];
      if(!DownloadModel(fname))
      {
         PrintFormat("Midas v7.0: FAILED to download %s — aborting", fname);
         return false;
      }
      ok++;
      Comment(StringFormat("Midas v7.0\n\n  Downloading models... %d/%d", ok, total));
   }
   // Exit model
   if(!DownloadModel("exit_v7.onnx"))
   { PrintFormat("Midas v7.0: FAILED exit_v7.onnx — aborting"); return false; }
   ok++;
   Comment(StringFormat("Midas v7.0\n\n  Downloading models... %d/%d", ok, total));
   // Meta-labeling head
   if(!DownloadModel("meta_v7.onnx"))
   { PrintFormat("Midas v7.0: FAILED meta_v7.onnx — aborting"); return false; }
   ok++;
   Comment(StringFormat("Midas v7.0\n\n  Downloading models... %d/%d", ok, total));
   ...
   {
      Print("Midas v7.0: regime_v4_xau.onnx unavailable on server — continuing without vol sizing");
   }
   PrintFormat("Midas v7.0: %d of %d models downloaded from server", ok, total);
   return true;
}
...
int OnInit()
{
   ...
   if(!ConfirmV7_Load())
   { Print("Midas v7: failed to load v7 confirmation models"); return INIT_FAILED; }
   if(!ExitModel_Load())
   { Print("Midas v7: failed to load exit model"); return INIT_FAILED; }
   if(!MetaModel_Load())
   { Print("Midas v7: failed to load meta-labeling model"); return INIT_FAILED; }
...
void OnTick()
{
   ...
   {
      if(InpVerbose)
         PrintFormat("v7 %s | regime=C%d %s | SKIP (non-tradeable)",
      ...
      if(g_bars_held >= MIN_HOLD_BARS)
      {
         ...
         if(CheckExitModel(rb, exit_prob))
         {
            for(int p=PositionsTotal()-1;p>=0;p--)
            {
               ...
               PrintFormat("v7 ML EXIT: prob=%.3f bars=%d", exit_prob, g_bars_held);
            }
   ...
   {
      if(dir == 0)
         PrintFormat("v7 %s | regime=C%d %s | no signal",
                     TimeToString(bt, TIME_DATE|TIME_MINUTES),
                     g_active_cluster, REGIME_NAMES[g_active_cluster]);
      else
         PrintFormat("v7 %s | regime=C%d %s | rule=%s dir=%+d",
                     TimeToString(bt, TIME_DATE|TIME_MINUTES),
                     g_active_cluster, REGIME_NAMES[g_active_cluster],
