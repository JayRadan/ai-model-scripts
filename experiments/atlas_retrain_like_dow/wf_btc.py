"""Walk-forward validation of BTC with the exact DOW recipe on 8-year data.
Expanding-window: for each quarterly test window, train on ALL prior data
(MFE>=2R candidates), predict, report PF@Q3. Robust if most windows are +ve."""
import sys, time
...
def labels_with_mfe(idxs,dirs,O,H,L,C,atr,sp,SL,TRAIL,MAXH,n):
    m=len(idxs); pnl=np.empty(m); mfe=np.empty(m)
    for k in range(m):
...
pnl,mfe=labels_with_mfe(idxs,dirs,O,H,L,C,atr,sp,SL,TRAIL,MAXH,n)
...
print(f"  candidates {len(idxs):,}  feats {len(feat_cols)}  ({time.time()-t0:.0f}s) — walk-forward:",flush=True)
...
print(f"  {'test window':<22}{'n':>6}{'WR%':>6}{'PF':>7}{'sumR':>9}")
pos=0; tot=0; allpf=[]
for ws in wins:
    we=ws+pd.DateOffset(months=3)
    tr=ctime<np.datetime64(ws); te=(ctime>=np.datetime64(ws))&(ctime<np.datetime64(we))
    trsel=tr&(mfe>=MFE_FILTER)
    if trsel.sum()<500 or te.sum()<30: continue
    q=fitpred(X[trsel],pnl[trsel].astype(np.float32),X[te])
    qm=q>=QTEST; rs=pnl[te][qm]; rs=rs[np.isfinite(rs)]
    if len(rs)<10: continue
    w,l=rs[rs>0],rs[rs<=0]; pf=w.sum()/max(-l.sum(),1e-9)
    pos+=pf>1.0; tot+=1; allpf.append(pf)
    print(f"  {str(ws.date())+'->'+str(we.date()):<22}{len(rs):>6}{(rs>0).mean()*100:>6.1f}{pf:>7.2f}{rs.sum():>+9.1f}")
print(f"\n  WALK-FORWARD: {pos}/{tot} windows PF>1.0  median PF={np.median(allpf):.2f}  ({time.time()-t0:.0f}s)")
