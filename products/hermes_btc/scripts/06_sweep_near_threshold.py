"""
NEAR sweep on M1 with orderflow + SL=4.
Sweep NEAR ∈ {0.10, 0.15, 0.20, 0.25, 0.35, 0.50, 0.75, 1.0}.
"""
...
NEAR_GRID = [0.10, 0.15, 0.20, 0.25, 0.35, 0.50, 0.75, 1.00]
...
def main():
    t0 = time.time()
    print(f"\n{'='*72}\n  BTC M1 NEAR sweep (SL={SL}R, TRAIL={TRAIL}R, Q≥{Q_THR}, with orderflow)\n{'='*72}", flush=True)

    print("\n[1/3] loading M1 + features ...", flush=True)
    ...
    n = len(df)
    spread_R = BTC_SPREAD_USD / np.nanmedian(atr)
    ...
    def run_for_near(NEAR):
        ...
        # Bare metrics on labels
        bare = metrics(pnl_s[test_m])
        ...
        return {"near": NEAR, "candidates": int(len(idxs)),
                "train_n": int(train_m.sum()), "test_n": int(test_m.sum()),
                "bare": bare, **m, "trd_per_day": m["n"]/span_days}

    print(f"\n[2/3] NEAR sweep ...", flush=True)
    print(f"  {'NEAR':>4}  {'cand':>6}  {'train':>6}  {'test':>6}  {'n':>5} {'WR%':>5} {'PF':>5} {'sumR':>8} {'DD':>6} {'avgR':>6} {'trd/d':>5}", flush=True)
    rows = []
    for NEAR in NEAR_GRID:
        r = run_for_near(NEAR)
        if not r: continue
        rows.append(r)
        print(f"  {NEAR:>4.2f}  {r['candidates']:>6,}  {r['train_n']:>6,}  {r['test_n']:>6,}  "
              f"{r['n']:>5,} {r['wr']*100:>5.1f} {r['pf']:>5.2f} {r['sum_r']:>+8.1f} "
              f"{r['max_dd_r']:>6.1f} {r['avg_r']:>+6.3f} {r['trd_per_day']:>5.2f}", flush=True)

    (HERE / "near_sweep_summary.json").write_text(json.dumps(rows, indent=2, default=str))
    print(f"\n  done in {time.time()-t0:.0f}s  wrote near_sweep_summary.json", flush=True)
