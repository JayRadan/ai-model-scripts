//+------------------------------------------------------------------+
//|  EdgePredictor_EURUSD.mq5                                        |
//|  EURUSD M5 — Rule + ML-confirmation system                       |
//|                                                                    |
//|  Architecture:                                                     |
...
//|   1  regime selector (constants in regime_selector_eurusd.mqh)    |
//|                                                                    |
...
#property description "AI-powered EURUSD rule + ML confirmation trading system."
#property description "Requires license key. Allow WebRequest for your server URL in MT5 settings."
#property strict

#include <Trade/Trade.mqh>
#include <regime_selector_eurusd.mqh>
#include <confirmation_router_eurusd.mqh>
...
input int    InpMaxSpread          = 30;    // skip entry if spread > N points (EURUSD tight)
input long   InpMagic              = 420306;
...
static const double TP_ATR_MULT    = 6.0;   // Meridian: widened from 2.0 → 6.0 (PF 1.51 → 1.73)
static const double SL_ATR_MULT    = 1.5;   // Meridian: widened from 1.0 → 1.5 (size /1.5 to keep $-risk)
...
void ReportStats()
{
   ...
   string json = StringFormat(
      "{\"ea_name\":\"Meridian\",\"account\":\"%s\",\"symbol\":\"%s\","
...
void DrawWatermark()
{
   ...
   ObjectSetString (chart, name, OBJPROP_TEXT,       "EdgePredictor Meridian  |  Jay  |  edgepredictor.pro");
...
void UpdateDashboard()
{
   ...
   string dash = "";
   dash += "═══════════════════════════════════════\n";
   dash += "   EdgePredictor Meridian  v1.0\n";
   dash += "   EURUSD · M5 · Rule + ML Confirmation\n";
   dash += "═══════════════════════════════════════\n";
   dash += "\n";
...
{
   // Walk backwards from idx, counting consecutive same-direction bars
...
//| Base features identical to gold v5; tech features match Python    |
//| pipeline's ALL_FEAT_COLS.                                          |
//+------------------------------------------------------------------+
...
void RefreshActiveRegime()
{
   ...
   Print("EU: active regime = C", g_active_cluster, " ", REGIME_NAMES[g_active_cluster],
...
int RefreshLicenseStatus(bool startup_check)
{
   ...
   {
      Print("EdgePredictor EURUSD: No license key provided. Paste your key in Inputs tab.");
      if(startup_check)
         Alert("EdgePredictor EURUSD\nLicense key missing\nPaste your license key in the Inputs tab.");
   ...
   if(res == 200)
   {
      ...
      if(startup_check)
         Print("EdgePredictor EURUSD: License validated successfully.");
   ...
   if(res == 403)
   {
      ...
      g_licensed = false;
      Print("EdgePredictor EURUSD: license inactive - ", message);
      if(startup_check)
         Alert("EdgePredictor EURUSD\nLicense: ", reason, "\n", message);
      return LICENSE_STATUS_INVALID;
   }
   ...
   g_licensed = false;
   PrintFormat("EdgePredictor EURUSD: license check failed (HTTP %d, err=%d)", res, GetLastError());
   if(startup_check)
...
bool ValidateLicense()
{
   ...
   {
      Comment("EdgePredictor EURUSD v1.0\n\n  License: ACTIVE\n  Status: Ready to trade");
      return true;
   }
   return false;
}
...
bool DownloadModel(string filename)
{
   ...
   if(res != 200)
   {
      PrintFormat("EdgePredictor EURUSD: model download HTTP %d for %s (err=%d)", res, filename, GetLastError());
      return false;
   }
   int fh = FileOpen(filename, FILE_WRITE | FILE_BIN);
   if(fh == INVALID_HANDLE)
   {
      PrintFormat("EdgePredictor EURUSD: cannot write '%s' (err=%d)", filename, GetLastError());
      return false;
   }
   FileWriteArray(fh, result, 0, ArraySize(result));
   FileClose(fh);
   PrintFormat("EdgePredictor EURUSD: '%s' downloaded (%d bytes)", filename, ArraySize(result));
   return true;
}
...
bool DownloadAllModels()
{
   ...
   for(int r = 0; r < RULE_COUNT; r++)
   {
      ...
      {
         PrintFormat("EdgePredictor EURUSD: FAILED to download %s - aborting", fname);
         return false;
      }
      ok++;
      Comment(StringFormat("EdgePredictor EURUSD v1.0\n\n  License: ACTIVE\n  Downloading models... %d/%d", ok, total));
   }
   // Vol-regime sizer — non-fatal: falls back to flat 1.0x lots if unavailable
   if(DownloadModel("regime_v4_eu.onnx"))
   {
      ok++;
      Comment(StringFormat("EdgePredictor EURUSD v1.0\n\n  License: ACTIVE\n  Downloading models... %d/%d", ok, total));
   }
   ...
   {
      Print("EdgePredictor EURUSD: regime_v4_eu.onnx unavailable on server — continuing without vol sizing");
   }
   PrintFormat("EdgePredictor EURUSD: %d of %d models downloaded from server", ok, total);
   return true;
}
...
int OnInit()
{
   // Auto-detect EURUSD variant
   if(StringFind(_Symbol, "EURUSD") < 0 && StringFind(_Symbol, "eurusd") < 0)
   {
      Alert("EdgePredictor Meridian\nPlease attach to a EURUSD chart.");
      return INIT_FAILED;
   }
   ...
   Comment("EdgePredictor Meridian v1.0\n\n  Validating license...");
   ...
   {
      Print("EdgePredictor Meridian: failed to load confirmation models");
      return INIT_FAILED;
   }
   // Vol-regime sizer (regime_v4) — non-fatal if it fails; falls back to 1.0× lots
   if(!RegimeV4_Load("regime_v4_eu.onnx"))
      Print("EdgePredictor Meridian: regime_v4 sizer unavailable — using flat 1.0× lots");
   ...
   PrintFormat("EdgePredictor Meridian ready | regime=C%d %s | refresh=%d bars long=%s short=%s",
               g_active_cluster, REGIME_NAMES[g_active_cluster],
...
void OnTick()
{
   ...
   {
      if(InpVerbose)
         PrintFormat("EU %s | regime=C%d %s | SKIP (non-tradeable)",
   ...
   {
      if(InpVerbose) Print("EU: CopyRates insufficient");
      return;
   }
   ...
   {
      if(InpVerbose)
         PrintFormat("EU: spread %d > %d, skipping", (int)spread_pts, InpMaxSpread);
      return;
   }
   ...
   {
      if(dir == 0)
         PrintFormat("EU %s | regime=C%d %s | no signal",
                     TimeToString(bt, TIME_DATE|TIME_MINUTES),
                     g_active_cluster, REGIME_NAMES[g_active_cluster]);
      else
         PrintFormat("EU %s | regime=C%d %s | rule=%s dir=%+d",
                     TimeToString(bt, TIME_DATE|TIME_MINUTES),
                     g_active_cluster, REGIME_NAMES[g_active_cluster],
   ...
   if(InpVerbose && vol_regime >= 0)
      PrintFormat("EU: vol=%s (p=%.2f) lot_mult=%.2fx base=%.2f final=%.2f",
   ...
   if(dir == +1)
   {
      ...
      ok = g_trade.Buy(lots, _Symbol, tick.ask, sl, tp,
                       StringFormat("EU-L %s", RULE_NAMES[rule_id]));
   }
   else
   {
      ...
      ok = g_trade.Sell(lots, _Symbol, tick.bid, sl, tp,
                        StringFormat("EU-S %s", RULE_NAMES[rule_id]));
   }
   if(!ok && InpVerbose)
      PrintFormat("EU: order failed (%d)", GetLastError());
}
//+------------------------------------------------------------------+
