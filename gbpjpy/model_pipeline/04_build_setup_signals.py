"""
GBPJPY M5 — Build setup signals (48 rules across 4 regimes).
Adapted for GBPJPY characteristics: wider spreads, Tokyo/London sessions,
...
# ═══════════════════════════════════════════════════════════════
# SESSION HELPERS — GBPJPY: Tokyo, London, NY, Tokyo/London overlap
# ═══════════════════════════════════════════════════════════════
def is_tokyo(t, i): return 0 <= t["hours"][i] <= 8       # Server time (UTC+0 approx)
def is_london(t, i): return 7 <= t["hours"][i] <= 16
def is_ny(t, i): return 13 <= t["hours"][i] <= 21
def is_tokyo_open(t, i): return 0 <= t["hours"][i] <= 3   # Early Tokyo
def is_london_open(t, i): return 7 <= t["hours"][i] <= 10
def is_ny_open(t, i): return 13 <= t["hours"][i] <= 16
def is_tokyo_london_overlap(t, i): return 7 <= t["hours"][i] <= 8  # Overlap zone
...
def r0k_tokyo_buy(t, i):
    """GBPJPY-specific: Tokyo session buy on pullback to SMA in uptrend"""
    return is_tokyo_open(t, i) and t["bullish"][i] and t["dist_sma20"][i] > -0.3 and t["dist_sma20"][i] < 0.3 and t["stoch_k"][i] < 0.4

def r0l_london_open_buy(t, i):
    """London open momentum buy — GBPJPY moves hard at London open"""
    return is_london_open(t, i) and t["bullish"][i] and t["dist_sma20"][i] > 0 and t["range_atr"][i] > 0.8
...
def r1l_session_break_buy(t, i):
    """Tokyo range breakout at London open — classic GBPJPY play"""
...
def r2k_london_sell(t, i):
    """London open sell — GBPJPY often dumps at London if JPY is strong"""
    return is_london_open(t, i) and not t["bullish"][i] and t["dist_sma20"][i] < 0 and t["stoch_k"][i] > 0.6

def r2l_tokyo_fade(t, i):
    """Fade Tokyo rally in downtrend — JPY crosses often reverse Tokyo moves"""
    return is_tokyo(t, i) and not t["bullish"][i] and t["rsi"][i] > 60 and t["dist_sma50"][i] < 0
...
def r3j_session_vol_buy(t, i):
    """Tokyo/London overlap volatility — GBPJPY's most explosive period"""
    return (is_london_open(t, i) and t["range_atr"][i] > 1.2 and
            t["bullish"][i] and t["stoch_k"][i] < 0.4)
def r3j_session_vol_sell(t, i):
    return (is_london_open(t, i) and t["range_atr"][i] > 1.2 and
            not t["bullish"][i] and t["stoch_k"][i] > 0.6)


# ═══════════════════════════════════════════════════════════════
# RULE REGISTRY — 48 rules total
# ═══════════════════════════════════════════════════════════════
RULES = [
    ...
    ("R0k_tokyo_buy",      0, "buy", r0k_tokyo_buy, None, 3),
    ("R0l_london_open_buy",0, "buy", r0l_london_open_buy, None, 3),
    ...
    ("R3l_tokyo_fade",     3, "sell", None, r2l_tokyo_fade, 3),
...
out_path = P.data("setup_signals_gbpjpy.csv")
