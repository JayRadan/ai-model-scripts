# hermes_btc — pipeline scripts

End-to-end scripts for the **hermes_btc** product, in execution order.
...
| 3 | `03_train_q_production.py` | Train the XGBRegressor Q-model on MFE≥2R candidates + write the .pkl bundle | `commercial/server/decision_engine/models/hermes_btc_validated.pkl` |
| 10 | `10_sim_today.py` | Simulate today's trades under the **deployed** config (Q≥4.0, trend_slope_block=1.5). Outputs trade-by-trade table + equity PNG | `scripts/_out/sim_today.png` |

## Deployed config (as of 2026-06-09)

| Parameter | Value | Source of truth |
|---|---|---|
| Q threshold | **4.0** | `commercial/server/decision_engine/configs/hermes_btc.py` |
| Filter | **trend_slope_block=1.5** | Same config |
...
python3 products/hermes_btc/scripts/03_train_q_production.py
```

After step 3, the new `hermes_btc_validated.pkl` is in the server's models/ directory.
**Commit it via the commercial repo** to deploy.

## How to verify a config change without redeploying

```bash
# Edit the config in commercial/server/decision_engine/configs/hermes_btc.py
...
in sync with `commercial/server/decision_engine/configs/hermes_btc.py` whenever
