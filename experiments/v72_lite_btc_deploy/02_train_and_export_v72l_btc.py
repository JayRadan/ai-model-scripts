...
Produces:
  models/confirm_v7_btc_c{cid}_{rule}.onnx   — 28 per-rule confirmation models
  models/exit_v7_btc.onnx                    — global ML exit model
  models/meta_v7_btc.onnx                    — meta-labeling head (input: 18 feats + direction + cid)
  models/v7_deploy_btc.json                  — feature order, thresholds, meta threshold
...
META_THRESHOLD = 0.525                      # validated on clean holdout
...
def load_swing_with_physics():
    ...
    mod = SourceFileLoader("p04b_btc", "/home/jay/Desktop/new-model-zigzag/experiments/v72_lite_btc_deploy/04b_compute_physics_features_btc.py").load_module()
...
def simulate(confirmed, swing, atr, exit_mdl):
    """Batched: build (N_trades*MAX_HOLD) feature matrix, one predict_proba call."""
    time_to_idx = pd.Series(swing.index.values, index=swing["time"].values)
...
def main():
    ...
    for (cid, rule), mdl in mdls.items():
        onnx_path = P.model(f"confirm_v7_btc_c{cid}_{rule}.onnx")
    ...
    # Exit
    exit_onnx = P.model("exit_v7_btc.onnx")
    ...
    print(f"  ✓ exit_v7_btc.onnx  feats={r_exit['n_features']}")
    # Meta
    meta_onnx = P.model("meta_v7_btc.onnx")
    ...
    print(f"  ✓ meta_v7_btc.onnx  feats={r_meta['n_features']}  diff={diff_str}")

    # Summary file
    out_summary = P.model("v7_deploy_btc.json")
