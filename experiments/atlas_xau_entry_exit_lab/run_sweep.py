...
for tr_s, te_s, te_e in WINS:
    ...
    for g in GRID:
        ...
        if g == (60, 1.0) and tk is not None: hold_stats["tt60"].append((xit_[tk] - eb_[tk]))
    ev(p_lm, *SIMS[(60, 1.0)], res["label_matched"])
...
json.dump({str(k): v for k, v in res.items()}, open(OUT / "sweep_results.json", "w"), default=str, indent=1)
log("sweep done")
