"""
GBPJPY honest holdout backtest — replays last 20% of each rule's setups
through the trained confirmation filter.

GBPJPY PnL: 1 pip = $0.066 per 0.01 lot (at ~190 JPY/USD rate).
TP=2*ATR ~40 pips avg, SL=1*ATR ~20 pips avg on GBPJPY M5.
Simplified: win=$2.64, loss=$1.32 per 0.01 lot (scaled by ATR).
We use the 2:1 ratio abstraction like EURUSD for consistency.
"""
import json
import numpy as np
import pandas as pd
...
# GBPJPY: 3 pip spread * 0.001 * 100000 / ~190 = ~$1.58/lot, for 0.01 lot ~$0.016
# Simplified: spread cost ~$0.20 per trade at 0.01 lot
SPREAD_PNL = 0.20
...
for rule_name in rules:
    ...
    # PnL: TP=2xATR, SL=1xATR ratio → win=$2, loss=$1 (normalized)
...
# Print results
print("\n" + "="*80)
print("GBPJPY HONEST HOLDOUT BACKTEST RESULTS")
print("="*80)
...
if all_times:
    ...
    with open(P.data("backtest_gbpjpy_summary.json"), "w") as f:
    ...
    plt.suptitle(
        "GBPJPY PER-RULE CONFIRMATION — HONEST HOLDOUT BACKTEST",
        color="#FFD700", fontsize=14, fontfamily="monospace",
        fontweight="bold", y=1.015,
    )
    plt.savefig(P.data("backtest_gbpjpy.png"), dpi=140, bbox_inches="tight", facecolor="#080c12")
    print(f"\nSaved: backtest_gbpjpy.png")
