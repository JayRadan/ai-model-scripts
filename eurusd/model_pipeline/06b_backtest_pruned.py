"""
EURUSD backtest with PRUNED rules — before vs after comparison.
"""
import json
import numpy as np
import pandas as pd
...
SPREAD_PNL = 1.5 * 0.00001 * 100000  # ~$0.15 per 0.01 lot
...
KEEP_RULES = {
    "R0e_sma_bounce", "R0g_close_streak", "R0k_london_buy", "R0l_doji_reversal",
    "R1b_stoch", "R1f_mean_revert", "R1j_engulfing", "R1n_range_fade",
    "R2a_swing_high", "R2b_lower_high", "R2e_sma_reject", "R2f_three_red",
    "R2h_pin_bar", "R2i_engulfing", "R2j_ema_reject", "R2k_london_sell",
    "R3d_stoch_vol", "R3j_session_vol",
}
...
print("\n" + "#"*80)
print("# EURUSD MERIDIAN — BEFORE vs AFTER PRUNING")
print("#"*80)

before = run_backtest(keep_only=None, label="BEFORE PRUNING (all 29 active rules)")
after = run_backtest(keep_only=KEEP_RULES, label="AFTER PRUNING (18 quality rules)")
...
if before and after:
    ...
    plt.suptitle(
        "EURUSD MERIDIAN — BEFORE vs AFTER RULE PRUNING",
        color="#FFD700", fontsize=14, fontfamily="monospace",
        fontweight="bold", y=1.02,
    )
    out = P.data("backtest_eurusd_pruned_comparison.png")
