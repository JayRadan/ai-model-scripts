"""v8.9 holdout validation: apply 4h flow direction filter to all
holdout trades and measure ΔR / ΔWR / ΔPF.

Method:
  1. Load swing_v5 XAU + BTC bars (full history through last available)
  2. Compute flow_4h for every M5 bar
  3. For each holdout trade (Oracle, Midas, BTC), look up flow_4h at
     entry time and check if sign(flow_4h) == direction
  4. Compute baseline vs filtered WR/PF/R

Pass bar:
  - Filtered ΔR ≥ -10% (cost of filter is small)
  - Filtered ΔWR_pp ≥ +2 (filter actually helps)
  - Filtered ΔPF ≥ +0.20
"""
...
def attach_flow(trades_path, swing_path):
    ...
    print(f"  swing: {len(swing)} bars  {swing.time.min()} → {swing.time.max()}", flush=True)

    print("  computing 4h flow...", flush=True)
    ...
    out = pd.merge_asof(trades, swing[["time", "flow_4h"]], on="time",
                          direction="backward",
                          tolerance=pd.Timedelta("10min"))
    out = out.dropna(subset=["flow_4h"]).reset_index(drop=True)
...
def evaluate(name, df):
    ...
    print(f"  4h-agree: n={new_n} WR={new_wr:.1%} PF={new_pf:.2f} R={new_R:+.0f}  "
...
def main():
    print("Loading XAU swing + computing flow_4h once...")
    print("--- Oracle XAU ---")
    ...
    df_o.to_csv(os.path.join(os.path.dirname(__file__), "holdout_oracle_xau.csv"), index=False)

    print("\n--- Midas XAU ---")
    df_m = attach_flow(os.path.join(ROOT, "data/v6_trades_holdout_xau.csv"),
                         os.path.join(ROOT, "data/swing_v5_xauusd.csv"))
    evaluate("Midas XAU", df_m)
    df_m.to_csv(os.path.join(os.path.dirname(__file__), "holdout_midas_xau.csv"), index=False)
    ...
    df_b.to_csv(os.path.join(os.path.dirname(__file__), "holdout_oracle_btc.csv"), index=False)
