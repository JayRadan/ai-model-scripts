...
COLORS = {0:"#FFD700", 1:"#00E5FF", 2:"#FF6B6B", 3:"#69FF94"}
...
def list_rule_models(cid: int):
    ...
    pattern = P.model(f"confirm_c{cid}_*.json")
    names = []
...
def plot_results(results):
    ...
    for col_idx, cid in enumerate([0, 1, 2, 3]):
        r = results[cid]
        color = COLORS[cid]
    ...
    out = P.data("backtest_confirmation.png")
    plt.savefig(out, dpi=140, bbox_inches="tight", facecolor="#080c12")
    print(f"\nSaved: {out}")


def main():
    results = {cid: run_cluster(cid) for cid in [0, 1, 2, 3, 4]}
    ...
    with open(P.data("backtest_confirmation_summary.json"), "w") as f:
        json.dump(summary, f, indent=2, default=float)
    print("Saved: " + P.data("backtest_confirmation_summary.json"))
