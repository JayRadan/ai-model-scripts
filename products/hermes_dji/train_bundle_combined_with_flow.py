"""Hermes DJI — train combined-Q with 43 features (29 std + 14 orderflow)."""
...
SL = 4.0; TRAIL = 3.0; MAX_HOLD = 300
NEAR = 0.50; COUNTER_THR = 1.5
DJI_SPREAD_USD = 2.0
OUT_PKL = HERE / "hermes_dji_validated_with_flow.pkl"
...
def main():
    print("="*72); print("  Hermes DJI — combined-Q with 43 features (M1 + orderflow)"); print("="*72)
    t0 = time.time()

    print("\n[1/5] loading M1 orderflow ...", flush=True)
    m1 = pd.read_parquet(ROOT / "data" / "m1_dji_orderflow.parquet")
    ...
    print(f"  features: {len(feat_cols)} (std={len(feats_std.columns)+4}, flow={len(feats_flow.columns)})", flush=True)
    print(f"  flow cols present: {list(feats_flow.columns)}")
    ...
    for qt in [0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 4.0, 5.0]:
        ...
        holdout_by_q[qt] = m
        if m:
            print(f"    Q≥{qt}: n={m['n']:>6,}  WR={m['wr']*100:>5.1f}%  PF={m['pf']:>6.2f}  "
