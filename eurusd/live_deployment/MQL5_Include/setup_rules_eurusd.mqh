//+------------------------------------------------------------------+
//| setup_rules_eurusd.mqh                                           |
//| Hand-coded setup rules — exact mirror of                          |
//| 04_build_setup_signals.py (EURUSD M5, 48 rules, 4 clusters).     |
//|                                                                    |
...
//+------------------------------------------------------------------+
#ifndef __SETUP_RULES_EURUSD_MQH__
#define __SETUP_RULES_EURUSD_MQH__

// Per-rule enum index — order MUST match confirmation_router_eurusd.mqh
enum RULE_ID
{
...
const string RULE_NAMES[RULE_COUNT] =
{
   ...
   "R0i_engulfing","R0j_ema_pullback","R0k_london_buy","R0l_doji_reversal",
   "R1a_bb","R1b_stoch","R2a_inside_break","R2b_squeeze",
   "R1e_double_touch","R1f_mean_revert","R1g_close_extreme","R2c_nr4_break",
   "R1i_pin_bar","R1j_engulfing","R2f_rsi_divergence","R2e_session_break",
   "R2d_3bar_reversal","R1n_range_fade",
   "R3a_swing_high","R3b_lower_high","R3c_bounce_fade","R3d_overbought",
   "R3e_sma_reject","R3f_three_red","R3g_false_breakout","R3h_pin_bar",
   "R3i_engulfing","R3j_ema_reject","R3k_london_sell","R3l_doji_cont",
...
{
   3,3,3,3,3,3,3,3,3,3,3,3,          // C0 x12
   3,3,3,3,3,3,3,3,                    // C1 x8
   3,3,3,3,3,3,                        // C2 x6
   3,3,3,3,3,3,3,3,3,3,3,3,          // C3 x12
   3,3,3,3,3,3,3,3,3,3               // C4 x10
};
...
//+------------------------------------------------------------------+
//| R0k: London open buy                                              |
//| Python: london_open and bullish and dist_sma20 > 0 and sk < 0.4  |
//+------------------------------------------------------------------+
int Rule_R0k_london_buy(const float &feat[], const MqlRates &rb[])
{
   if(!IsLondonOpen(rb, 0)) return 0;
   double dsma20 = (double)feat[FI_DIST_SMA20];
   double sk = (double)feat[FI_STOCH_K];
   if(IsBullish(rb, 0) && dsma20 > 0 && sk < 0.4)
      return +1;
   return 0;
}

//+------------------------------------------------------------------+
//| R0l: Doji reversal (buy)                                          |
//| Python: body_ratio[i-1] < 0.15 and !bull[i-1] and bull[i]        |
//|         and body_ratio[i] > 0.4 and stoch_k < 0.35               |
//+------------------------------------------------------------------+
int Rule_R0l_doji_reversal(const float &feat[], const MqlRates &rb[])
{
   if(ArraySize(rb) < 2) return 0;
   if(BarBodyRatio(rb, 1) < 0.15 && !IsBullish(rb, 1) &&
      IsBullish(rb, 0) && BarBodyRatio(rb, 0) > 0.4 &&
      (double)feat[FI_STOCH_K] < 0.35)
      return +1;
   return 0;
}
...
//+------------------------------------------------------------------+
//| R2l: Doji continuation (sell)                                     |
//| Python: body_ratio[i-1] < 0.15 and bull[i-1] and !bull[i]        |
//|         and body_ratio[i] > 0.4 and stoch_k > 0.65               |
//+------------------------------------------------------------------+
int Rule_R3l_doji_cont(const float &feat[], const MqlRates &rb[])
{
   if(ArraySize(rb) < 2) return 0;
   if(BarBodyRatio(rb, 1) < 0.15 && IsBullish(rb, 1) &&
      !IsBullish(rb, 0) && BarBodyRatio(rb, 0) > 0.4 &&
      (double)feat[FI_STOCH_K] > 0.65)
      return -1;
   return 0;
}
...
//+------------------------------------------------------------------+
//| R3j: Session vol (buy/sell) — NY open with vol                    |
//| Buy: NY open, range_atr > 1.2, bullish, stoch_k < 0.4           |
//| Sell: NY open, range_atr > 1.2, !bullish, stoch_k > 0.6         |
//+------------------------------------------------------------------+
int Rule_R4j_session_vol(const float &feat[], const MqlRates &rb[])
{
   if(!IsNYOpen(rb, 0)) return 0;
   double ra = (double)feat[FI_RANGE_ATR];
   double sk = (double)feat[FI_STOCH_K];
...
int CheckRule(int rule_id, const float &feat[], const MqlRates &rb[])
{
   switch(rule_id)
   {
      ...
      case RULE_R0k_london_buy:     return Rule_R0k_london_buy(feat, rb);
      case RULE_R0l_doji_reversal:  return Rule_R0l_doji_reversal(feat, rb);
      ...
      case RULE_R3l_doji_cont:      return Rule_R3l_doji_cont(feat, rb);
...
#endif  // __SETUP_RULES_EURUSD_MQH__
