"""
BTC M1 pipeline on FULL Dukascopy M1 data (2018-2026).
...
def main():
    t0 = time.time()
    print(f"\n{'='*72}\n  BTC M1 FULL — 2018-2026 Dukascopy\n{'='*72}", flush=True)
    ...
    print(f"  bars={n:,}  median ATR={median_atr:.3f}  spread={BTC_SPREAD_USD}$ ≈ {spread_R:.3f}R", flush=True)
    ...
    print(f"\n  Q sweep — NO SPREAD vs WITH SPREAD ({BTC_SPREAD_USD}$ ≈ {spread_R:.3f}R/trade):")
    ...
    if best:
        ...
        ex_df.to_csv(out_dir / "btc_m1_full_q1.0_trades.csv", index=False)
        # Equity plot
        import matplotlib
        matplotlib.use("Agg")
        ...
        a0.set_title(f"BTC M1 FULL — Q≥1.0 with spread ({BTC_SPREAD_USD}$) — n={m['n']} PF={m['pf']:.2f} sumR={m['sum_r']:+.0f}R DD={m['max_dd_r']:.0f}R", fontsize=12)
        ...
        plt.tight_layout()
        plt.savefig(out_dir / "btc_m1_full_q1.0_equity.png", dpi=110)
        plt.close()
        print(f"\n  wrote btc_m1_full_q1.0_trades.csv + equity.png", flush=True)
