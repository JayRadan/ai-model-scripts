"""
Hermes DJI — train + freeze the production Q-regressor bundle (M1 only, no orderflow).

Architecture: combined-Q (pullback ≤0.5 ATR OR counter ≥1.5 ATR wrong-side),
              29 standard features (no orderflow yet).
Train cutoff: 2025-09-01 (research holdout split, matches XAU bundle).
Label: forward pnl_R with SL=4×ATR, trail=3×ATR, max_hold=300, spread-adjusted.

Output: products/hermes_dji/hermes_dji_validated.pkl (and copied to commercial server
        only if user confirms holdout is acceptable).
"""
...
# Reuse the same standard-feature engineering as XAU
...
NEAR = 0.50
COUNTER_THR = 1.5
DJI_SPREAD_USD = 2.0   # typical Dow CFD spread (conservative)

OUT_PKL = HERE / "hermes_dji_validated.pkl"
...
def main():
    print("="*72)
    print("  Hermes DJI — bundle training (combined-Q, M1 only)")
    print("="*72)
    t0 = time.time()

    print("\n[1/5] loading M1 ...", flush=True)
    ...
    print(f"  bars={n:,}  median_atr={median_atr:.3f}  spread={DJI_SPREAD_USD}$ ≈ {spread_R:.3f}R", flush=True)
    ...
    X_all = pd.concat([extra.reset_index(drop=True), feats_std], axis=1)
    ...
    for qt in [0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 4.0, 5.0]:
        m = metrics(test_pnl[q_te >= qt])
        holdout_by_q[qt] = m
        if m:
            print(f"    Q≥{qt}: n={m['n']:>6,}  WR={m['wr']*100:>5.1f}%  PF={m['pf']:.2f}  "
                  f"sumR={m['sum_r']:>+9.0f}  DD={m['max_dd_r']:>6.0f}  avgR={m['avg_r']:+.3f}", flush=True)

    # Setup breakdown at Q≥1
    pullback_test = pullback_mask[idxs][test_m]
    counter_test = counter_mask[idxs][test_m]
    sel = q_te >= 1.0
    print(f"\n  setup breakdown at Q≥1.0:")
    for name, msk in [("pullback", pullback_test), ("counter", counter_test)]:
        m = metrics(test_pnl[sel & msk])
        if m:
            print(f"    {name:>9}: n={m['n']:>6,}  WR={m['wr']*100:.1f}%  PF={m['pf']:.2f}  sumR={m['sum_r']:+.0f}")
