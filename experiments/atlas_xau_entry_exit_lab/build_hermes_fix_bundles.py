...
def build(name, parquet, instr, bm, maxh, target_per_day, cd, trading_days_frac, cols=None):
    ...
    _, _, xit_live = sim_tt(idx, dirs, O, H, L, C, atr, n, 7.0, 2.0, maxh, 30, 0.75)     # live exits for calibration
    ...
    payload = {"version": f"edge_pullback_v3_tt30{'m5' if bm > 1 else ''}_{name}",
               "q_model": m, "feat_cols": FC, "threshold": float(thr), "near_thr": 1.0,
               "sl_R": 7.0, "trail_R": 2.0, "be_r": 0.0, "maxh": maxh,
               "tight_after": 30, "tight_trail_R": 0.75, "bar_minutes": bm,
               ...
               "recipe": f"pullback |dist_tfk|<=1.0 dir=committed_dir on M{bm}, XGB gross R "
                         ...
                         f"tt-trail 2->0.75*ATR after 30 M{bm} bars"}
    out = MODELS / f"{name}_validated.pkl"
    bak = MODELS / f"{name}_validated.pkl.bak_pre_edge_fix_2026-07-07"
...
build("hermes_btc", "/home/jay/Desktop/new-model-zigzag/data/m1_btc_orderflow_8y.parquet",
      INSTRUMENT_VCCY_BTC_USD, bm=1, maxh=300, target_per_day=11.0, cd=5, trading_days_frac=1.0,
      cols=["time", "open", "high", "low", "close", "tick_volume"])
build("hermes_xau", "/home/jay/Desktop/new-model-zigzag/data/m1_xau_full.parquet",
      INSTRUMENT_FX_METALS_XAU_USD, bm=5, maxh=60, target_per_day=5.0, cd=3, trading_days_frac=5/7)
log("bundles built")
