...
CFGS = [
    # (name, parquet, cols, bm, maxh, tt_grid, target/day, spreads, spread_mode, days_frac)
    ("dji_m5",  "/home/jay/Desktop/new-model-zigzag/data/m1_dji_full.parquet", None, 5, 60,
     [(0, 2.0), (6, 0.75), (15, 0.75), (30, 0.75)], 5.0, [1.0, 1.5, 2.0], "pts", 5/7),
    ("dji_m15", "/home/jay/Desktop/new-model-zigzag/data/m1_dji_full.parquet", None, 15, 20,
     [(0, 2.0), (5, 0.75), (10, 0.75), (15, 0.75)], 3.0, [1.0, 1.5, 2.0], "pts", 5/7),
    ("btc_m5",  "/home/jay/Desktop/new-model-zigzag/data/m1_btc_orderflow_8y.parquet",
     ["time", "open", "high", "low", "close", "tick_volume"], 5, 60,
     [(0, 2.0), (6, 0.75), (15, 0.75), (30, 0.75)], 5.0, [0.1, 0.2, 0.3], "R", 1.0),
]
...
for name, pq, cols, bm, MAXH, GRID, TPD, SPREADS, smode, dfrac in CFGS:
    ...
    for tr_s, te_s, te_e in WINS:
        ...
        for g in GRID:
            ...
            hs = SPREADS[1]
            for et_, xt_, r_, a_ in zip(times[eb_[tk]], times[xit_[tk]], R - hs * cost, sig_atr[tk]):
                streams[g].append((str(et_), str(xt_), float(r_), float(a_)))
...
json.dump(ALL, open(OUT / "multi_tf_results.json", "w"), default=str)
log("multi-tf labs done")
