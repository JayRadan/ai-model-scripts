...
#include <regime_selector_btc.mqh>
#include <confirmation_router_v7_btc.mqh>
...
input int    InpMaxSpread          = 5000;    // skip entry if spread > N points (BTC wider than gold)
input long   InpMagic              = 420805;   // BTC — distinct from v6/v7 for reporter filter
...
//--- constants
static const int    FEATURE_DIM    = 18;     // v7 = 14 v6 features + 4 v7 features (vpin, quad_var, har_rv, hawkes)
static const int    EXIT_FEAT_DIM  = 11;     // exit model unchanged (11 features)
static const int    META_FEAT_DIM  = 20;     // meta model: 18 features + direction + cid
static const double META_THRESHOLD = 0.525;  // BTC meta-labeling threshold (López de Prado)
static const int    RULE_FEAT_DIM  = 36;     // old features for rule detection (unchanged)
...
void DrawWatermark()
{
   ...
   ObjectSetString (chart, name, OBJPROP_TEXT,       "EdgePredictor BTC  |  Jay  |  edgepredictor.pro");
...
void UpdateDashboard()
{
   ...
   string dash = "";
   dash += "═══════════════════════════════════════\n";
   dash += "   EdgePredictor BTC  v1.0\n";
   dash += "   BTCUSD · M5 · Rule + ML Confirmation\n";
   dash += "═══════════════════════════════════════\n";
   dash += "\n";
...
void ReportStats()
{
   ...
   string json = StringFormat(
      "{\"ea_name\":\"Oracle\",\"account\":\"%s\",\"symbol\":\"%s\","
...
{
   // Match Python compute_hurst_rs: seg = ret[i-window:i] — window log returns
   // from prices c[i-window-1..i-1], EXCLUDES current bar.
   // In series: bar i-1 = rb[1], bar i-window-1 = rb[window+1]. Need window+1 prices.
   if(idx + window + 1 >= ArraySize(rb)) return 0.5;
   double rets[]; ArrayResize(rets, window);
   double sum_ret = 0;
   for(int k = 0; k < window; k++)
   {
      // Chronological order: k=0 oldest return, k=window-1 newest (excl. current)
      // Return between bar (i-window+k-1) and (i-window+k)
      int ri_old = window - k + 1;   // series index of older bar
      int ri_new = window - k;       // series index of newer bar
      double c_old = rb[idx + ri_old].close;
      double c_new = rb[idx + ri_new].close;
      rets[k] = (c_old > 1e-10) ? MathLog(c_new / c_old) : 0;
      sum_ret += rets[k];
   }
   ...
   for(int k = 0; k < window; k++)
   {
      ...
      ss += (rets[k] - mean_ret) * (rets[k] - mean_ret);
   }
   double R = maxC - minC, S = MathSqrt(ss / window);   // ddof=0 to match Python seg.std()
...
{
   // Match Python compute_ou_theta: x = cumsum(ret); seg = x[i-window:i]
   // seg has window cumulative values from bar (i-window) to bar (i-1), EXCLUDES current.
   // dx = diff(seg) = window-1 log returns; xm = seg[:-1] - mu.
   if(idx + window + 1 >= ArraySize(rb)) return 0.0;
   double x[]; ArrayResize(x, window);
   // x[k] = chronological cumulative value at bar (i-window+k). Anchor x[0]=0.
   x[0] = 0;
   for(int k = 1; k < window; k++)
   {
      int ri_old = window - k + 1;   // series index of older bar (one step back)
      int ri_new = window - k;       // series index of newer bar
      double c_old = rb[idx + ri_old].close;
      double c_new = rb[idx + ri_new].close;
      double lr = (c_old > 1e-10) ? MathLog(c_new / c_old) : 0;
      x[k] = x[k-1] + lr;
   }
...
{
   // Match Python compute_entropy: seg = ret[i-window:i] — window log returns,
   // using prices c[i-window-1..i-1], EXCLUDES current bar.
   // np.histogram with 10 bins (right-closed on last edge).
   if(idx + window + 1 >= ArraySize(rb)) return 1.0;
   double rets[]; ArrayResize(rets, window);
   for(int k = 0; k < window; k++)
   {
      int ri_old = window - k + 1;
      int ri_new = window - k;
      double c_old = rb[idx + ri_old].close;
      double c_new = rb[idx + ri_new].close;
      rets[k] = (c_old > 1e-10) ? MathLog(c_new / c_old) : 0;
   ...
   for(int k = 1; k < window; k++) { if(rets[k] < mn) mn = rets[k]; if(rets[k] > mx) mx = rets[k]; }
   if(mx - mn < 1e-15) return 1.0;
   int nbins = 10; int counts[]; ArrayResize(counts, nbins); ArrayInitialize(counts, 0);
   // np.histogram with 10 bins: bin edges uniform on [mn, mx]; last bin includes mx
   double bin_width = (mx - mn) / nbins;
   for(int k = 0; k < window; k++)
   {
      int b = (int)((rets[k] - mn) / bin_width);
      if(b >= nbins) b = nbins - 1;
      if(b < 0) b = 0;
      counts[b]++;
   }
   double ent = 0, total = (double)window;
   for(int b = 0; b < nbins; b++)
      if(counts[b] > 0) { double p = counts[b]/total; ent -= p*MathLog(p)/MathLog(2.0); }
   return ent / (MathLog((double)nbins)/MathLog(2.0));
}

double KramersEscape(const MqlRates &rb[], int idx, int window=100)
{
   // Match Python compute_kramers_up: seg = c[i-window:i] — window prices BEFORE bar i.
   // hi = seg.max(). sigma = np.std(np.diff(np.log(seg))) — ddof=0 on window-1 returns.
   // esc = exp(-(hi - c[i]) / (sigma * c[i])) — can exceed 1 if c[i] > hi.
   if(idx + window >= ArraySize(rb)) return 0.5;
   // seg bars: i-window..i-1 → series indices rb[1]..rb[window]
   double hi = rb[idx+1].close;
   for(int k = 1; k <= window; k++) if(rb[idx+k].close > hi) hi = rb[idx+k].close;
   // Log returns within seg (window-1 returns from window prices)
   double mean_r = 0, ss = 0;
   double rets[]; ArrayResize(rets, window-1);
   for(int k = 0; k < window-1; k++)
   {
      int ri_old = window - k;       // older bar in series
      int ri_new = window - k - 1;   // newer bar
      double c_old = rb[idx + ri_old].close;
      double c_new = rb[idx + ri_new].close;
      rets[k] = (c_old > 1e-10) ? MathLog(c_new / c_old) : 0;
      mean_r += rets[k];
   ...
   mean_r /= (window-1);
   for(int k = 0; k < window-1; k++) ss += (rets[k] - mean_r) * (rets[k] - mean_r);
   double sigma = MathSqrt(ss / (window-1));   // np.std default ddof=0 on window-1 values
   double c_cur = rb[idx].close;
   if(sigma < 1e-12 || c_cur < 1e-10) return 0.5;
   return MathExp(-(hi - c_cur) / (sigma * c_cur + 1e-10));
}

double WaveletER(const MqlRates &rb[], int idx, int window=120)
{
   // Match Python compute_wavelet_er: seg = c[i-window:i] — prices BEFORE bar i.
   // sn = (seg - seg.mean()) / (seg.std() + 1e-10)   (ddof=0)
   // ct = np.convolve(sn, kt, 'same'); cn = np.convolve(sn, kn, 'same')
   // er = mean(ct^2) / mean(cn^2)
   if(idx + window >= ArraySize(rb)) return 1.0;
   // seg in chronological order (oldest first): rb[window], rb[window-1], ..., rb[1]
   double seg[]; ArrayResize(seg, window);
   double seg_mean = 0;
   for(int k = 0; k < window; k++)
   { seg[k] = rb[idx + window - k].close; seg_mean += seg[k]; }
   seg_mean /= window;
   double seg_var = 0;
   for(int k = 0; k < window; k++) seg_var += (seg[k]-seg_mean)*(seg[k]-seg_mean);
   double seg_std = MathSqrt(seg_var / window);   // ddof=0 to match Python .std()
   double denom = seg_std + 1e-10;                 // Python uses seg.std()+1e-10
   for(int k = 0; k < window; k++) seg[k] = (seg[k] - seg_mean) / denom;

   int pt = (200 < window) ? 200 : window;
   double kt[]; ArrayResize(kt, pt);
   double a20 = 20.0, A20 = 2.0/(MathSqrt(3*a20)*MathPow(M_PI,0.25));
   for(int i=0; i<pt; i++) { double v = i - (pt-1.0)/2; kt[i] = A20*(1-v*v/(a20*a20))*MathExp(-v*v/(2*a20*a20)); }
   int pn = (30 < window) ? 30 : window;
   double kn[]; ArrayResize(kn, pn);
   double a3 = 3.0, A3 = 2.0/(MathSqrt(3*a3)*MathPow(M_PI,0.25));
   for(int i=0; i<pn; i++) { double v = i - (pn-1.0)/2; kn[i] = A3*(1-v*v/(a3*a3))*MathExp(-v*v/(2*a3*a3)); }

   // np.convolve mode='same' uses offset (K-1)/2. Kernel is symmetric so K//2 is equivalent.
   ...
   for(int i = 0; i < window; i++)
   {
      double ct = 0;
      for(int j = 0; j < pt; j++)
      { int si = i - ct_off + j; if(si >= 0 && si < window) ct += seg[si]*kt[j]; }
      e_trend += ct * ct;
      double cn = 0;
      for(int j = 0; j < pn; j++)
      { int si = i - cn_off + j; if(si >= 0 && si < window) cn += seg[si]*kn[j]; }
      e_noise += cn * cn;
   }
   return (e_noise > 1e-15) ? (e_trend/window) / (e_noise/window + 1e-15) : 1.0;
}
...
double ComputeVWAPDist(const MqlRates &rb[], int idx, double atr)
{
   ...
   double vol = MathMax((double)rb[idx].spread, 1.0);  // spread, not tick_volume — matches Python
...
{
   // Need enough history for ha_open (idx+1) and vol_lb rolling window at oldest point.
   // Warmup=100 bars gives negligible residual of EMA initial value: (1-alpha)^100 ≈ 5e-5.
   int N = ArraySize(rb);
   int warmup = 100;                                 // fixed — matches verification script
   if(idx + warmup + vol_lb >= N) return 0.0;

   double alpha = 2.0/(lookback+1.0);

   // Compute raw = trend_force * vol_factor * 1000 at each bar from oldest to newest,
   // then step EMA forward (matches Python pd.Series.ewm).
   // Oldest bar in our EMA window = idx + warmup; walk down to idx.
   double ema = 0.0;
   bool ema_init = false;
   for(int k = warmup; k >= 0; k--)
   {
      int i = idx + k;
      double ha_c = (rb[i].open + rb[i].high + rb[i].low + rb[i].close)/4.0;
      double ha_o = (i+1 < N) ? (rb[i+1].open + rb[i+1].close)/2.0 : rb[i].open;
      double tf = ha_c - ha_o;

      // Rolling mean spread over vol_lb bars INCLUDING bar i
      double avg_v = 0.0; int cnt = 0;
      for(int j = i; j < i + vol_lb && j < N; j++)
      { avg_v += MathMax((double)rb[j].spread, 1.0); cnt++; }
      if(cnt > 0) avg_v /= cnt; else avg_v = 1.0;
      double vf = (avg_v > 1e-10) ? MathMax((double)rb[i].spread, 1.0)/avg_v : 0.0;

      double raw_i = tf * vf * 1000.0;
      if(!ema_init) { ema = raw_i; ema_init = true; }
      else          { ema = alpha * raw_i + (1.0 - alpha) * ema; }
   }
...
//+------------------------------------------------------------------+
//| V7 BuildFeatures: 14 v6 physics+quantum features +               |
//|                    4 new v7 features (VPIN, quad_var, HAR, Hawkes)|
//+------------------------------------------------------------------+
bool BuildFeatures(const MqlRates &rb[], float &feat[])
{
   ...
   // ── Positions 0..13: identical to v6 ──────────────────────────
   ...
   int h4n = CopyRates(_Symbol, PERIOD_H4, 1, 150, h4);
   double qf_h4 = (h4n >= 125) ? QuantumFlowCalc(h4, 0, 21, 20) : 0.0;
   feat[9]  = (float)qf_h4;
   ...
   // ── Positions 14..17: v7 new features (6-bar smoothed) ──────────
   // v7_features.mqh maintains a ring buffer keyed on bar time so repeated
   // on-tick calls within the same bar don't double-push to the smoother.
   double vpin, qv, har, hawkes;
   ComputeV7Features(rb, rb[0].time, vpin, qv, har, hawkes);
   feat[14] = (float)vpin;
   feat[15] = (float)qv;
   feat[16] = (float)har;
   feat[17] = (float)hawkes;
   return true;
}
...
bool ExitModel_Load()
{
   g_exit_handle = OnnxCreate("exit_v7_btc.onnx", ONNX_DEFAULT);
   if(g_exit_handle == INVALID_HANDLE)
   { Print("BTC ExitModel: failed to load exit_v7_btc.onnx err=", GetLastError()); return false; }
   ...
   { Print("BTC ExitModel: shape fail"); return false; }
   Print("BTC ExitModel: exit_v7_btc.onnx loaded"); return true;
}
...
//+------------------------------------------------------------------+
//| Meta model (v7 López de Prado meta-labeling head)                |
//| Input:  20 features = 18 v7 features + direction + cid           |
//| Output: P(win) — take trade only if >= META_THRESHOLD (0.675)    |
//+------------------------------------------------------------------+
long g_meta_handle = INVALID_HANDLE;

bool MetaModel_Load()
{
   g_meta_handle = OnnxCreate("meta_v7_btc.onnx", ONNX_DEFAULT);
   if(g_meta_handle == INVALID_HANDLE)
   { Print("BTC MetaModel: failed to load meta_v7_btc.onnx err=", GetLastError()); return false; }
   ulong in_shape[]={1,META_FEAT_DIM}; ulong lbl_shape[]={1}; ulong prob_shape[]={1,2};
   if(!OnnxSetInputShape(g_meta_handle,0,in_shape) ||
      !OnnxSetOutputShape(g_meta_handle,0,lbl_shape) ||
      !OnnxSetOutputShape(g_meta_handle,1,prob_shape))
   { Print("BTC MetaModel: shape fail"); return false; }
   Print("BTC MetaModel: meta_v7_btc.onnx loaded (threshold=", META_THRESHOLD, ")");
   return true;
}
void MetaModel_Release() { if(g_meta_handle!=INVALID_HANDLE && g_meta_handle>0) { OnnxRelease(g_meta_handle); g_meta_handle=INVALID_HANDLE; } }

// Run meta-label check on a confirmed setup candidate.
// direction: +1 buy / -1 sell.  cid: active cluster (0..4).
// Returns true iff P(win) >= META_THRESHOLD.
bool MetaModel_Accept(const float &feat_v7[], int direction, int cid, double &out_pwin)
{
   out_pwin = 0.0;
   if(g_meta_handle == INVALID_HANDLE) return false;
   float in_data[]; ArrayResize(in_data, META_FEAT_DIM);
   ArrayCopy(in_data, feat_v7, 0, 0, FEATURE_DIM);         // 18 v7 features
   in_data[FEATURE_DIM + 0] = (float)direction;
   in_data[FEATURE_DIM + 1] = (float)cid;
   long out_label[1]; float probs[2];
   if(!OnnxRun(g_meta_handle, ONNX_DEFAULT, in_data, out_label, probs))
   { Print("BTC MetaModel: OnnxRun failed"); return false; }
   out_pwin = (double)probs[1];
   return out_pwin >= META_THRESHOLD;
}
...
bool CheckExitModel(const MqlRates &rb[], double &exit_prob)
{
   ...
   MqlRates h4[]; ArraySetAsSeries(h4,true);
   int h4n=CopyRates(_Symbol,PERIOD_H4,1,150,h4);
   in_data[9]=(float)((h4n>=125)?QuantumFlowCalc(h4,0,21,20):0.0);
   in_data[10]=(float)ComputeVWAPDist(rb,0,RawATR14(rb,0));
   long out_label[1]; float probs[2];
   if(!OnnxRun(g_exit_handle,ONNX_DEFAULT,in_data,out_label,probs)) { Print("BTC ExitModel: OnnxRun failed"); return false; }
...
void RefreshActiveRegime()
{
   ...
   Print("BTC: active regime = C", g_active_cluster, " ", REGIME_NAMES[g_active_cluster],
...
//+------------------------------------------------------------------+
int ScanAndConfirm(const float &feat_rules[], const float &feat_v7[], const MqlRates &rb[], int &out_rule)
{
   out_rule = -1;
   int best_dir = 0;
   double best_prob = 0.0;
   int best_candidate_dir = 0;
   int best_candidate_rule = -1;

   // Stage 1: per-rule XGBoost confirmation
   for(int r = 0; r < RULE_COUNT; r++)
   {
      ...
      double prob = 0.0;
      bool pass = ConfirmV7_BTC_Rule(r, feat_v7, prob);
      if(!pass) continue;

      if(prob > best_prob)
      {
         best_prob = prob;
         best_candidate_dir  = dir;
         best_candidate_rule = r;
      }
   }

   // Stage 2: meta-label gate — only accept if P_meta(win) >= META_THRESHOLD
   if(best_candidate_dir != 0 && best_candidate_rule >= 0)
   {
      double pwin = 0.0;
      bool meta_ok = MetaModel_Accept(feat_v7, best_candidate_dir, g_active_cluster, pwin);
      if(meta_ok)
      {
         best_dir = best_candidate_dir;
         out_rule = best_candidate_rule;
      }
      else
      {
         // Suppressed by meta — log once per occurrence for visibility
         PrintFormat("V7 META reject: rule=%s dir=%+d cid=%d P_conf=%.3f P_win=%.3f (thr=%.3f)",
                     RULE_NAMES[best_candidate_rule], best_candidate_dir,
                     g_active_cluster, best_prob, pwin, META_THRESHOLD);
      }
   }
   return best_dir;
}
...
bool DownloadAllModels()
{
   ...
   int ok = 0;
   int total = RULE_COUNT + 3; // per-rule ONNX + exit_v7 + meta_v7 + regime_v4
   for(int r = 0; r < RULE_COUNT; r++)
   {
      if(RULE_DISABLED_V7_BTC[r]) { ok++; continue; }              // skip disabled rules
      string fname = RULE_ONNX_V7_BTC[r];
      if(!DownloadModel(fname))
      {
         PrintFormat("BTC: FAILED to download %s — aborting", fname);
         return false;
      }
      ok++;
      Comment(StringFormat("BTC\n\n  Downloading models... %d/%d", ok, total));
   }
   // Exit model
   if(!DownloadModel("exit_v7_btc.onnx"))
   { PrintFormat("BTC: FAILED exit_v7_btc.onnx — aborting"); return false; }
   ok++;
   Comment(StringFormat("BTC\n\n  Downloading models... %d/%d", ok, total));
   // Meta-labeling head
   if(!DownloadModel("meta_v7_btc.onnx"))
   { PrintFormat("BTC: FAILED meta_v7_btc.onnx — aborting"); return false; }
   ok++;
   Comment(StringFormat("BTC\n\n  Downloading models... %d/%d", ok, total));
   // Vol-regime sizer — non-fatal: if server is older, EA falls back to 1.0x lot mult
   if(false) // BTC skips vol-regime sizer (XAU-specific)
   {
      ok++;
      Comment(StringFormat("BTC\n\n  Downloading models... %d/%d", ok, total));
   }
   ...
   {
      Print("BTC: regime_v4_xau.onnx unavailable on server — continuing without vol sizing");
   }
   PrintFormat("BTC: %d of %d models downloaded from server", ok, total);
   return true;
}
...
int OnInit()
{
   ...
   if(!ConfirmV7_BTC_Load())
   { Print("BTC: failed to load v7 confirmation models"); return INIT_FAILED; }
   if(!ExitModel_Load())
   { Print("BTC: failed to load exit model"); return INIT_FAILED; }
   if(!MetaModel_Load())
   { Print("BTC: failed to load meta-labeling model"); return INIT_FAILED; }
   V7_ResetSmoothingState();       // clear 6-bar rolling buffers on fresh start
   // Vol-regime sizer (regime_v4) — non-fatal if it fails; falls back to 1.0×
   if(!RegimeV4_Load("regime_v4_xau.onnx"))
      Print("EdgePredictor BTC: regime_v4 sizer unavailable — using flat 1.0× lots");
   ...
   for(int p = PositionsTotal() - 1; p >= 0; p--)
   {
      ...
      PrintFormat("BTC RECOVERED open position: ticket=%I64u dir=%+d entry=%.2f atr=%.2f bars_held=%d",
   ...
   PrintFormat("EdgePredictor BTC ready | regime=C%d %s | refresh=daily (00:00 UTC) long=%s short=%s",
               g_active_cluster, REGIME_NAMES[g_active_cluster],
...
void OnTick()
{
   ...
   {
      if(InpVerbose)
         PrintFormat("BTC %s | regime=C%d %s | SKIP (non-tradeable)",
   ...
   {
      if(InpVerbose) Print("BTC: CopyRates insufficient");
      return;
   }
   ...
   {
      if(InpVerbose)
         PrintFormat("BTC: spread %d > %d, skipping", (int)spread_pts, InpMaxSpread);
      return;
   }
   ...
   float feat_v7[];
   if(!BuildFeatures(rb, feat_v7)) return;
      ...
      if(g_bars_held >= MIN_HOLD_BARS)
      {
         ...
         if(CheckExitModel(rb, exit_prob))
         {
            for(int p=PositionsTotal()-1;p>=0;p--)
            {
               ...
               PrintFormat("BTC ML EXIT: prob=%.3f bars=%d", exit_prob, g_bars_held);
            }
   ...
   int rule_id = -1;
   int dir = ScanAndConfirm(feat_rules, feat_v7, rb, rule_id);
   ...
   {
      if(dir == 0)
         PrintFormat("BTC %s | regime=C%d %s | no signal",
                     TimeToString(bt, TIME_DATE|TIME_MINUTES),
                     g_active_cluster, REGIME_NAMES[g_active_cluster]);
      else
         PrintFormat("BTC %s | regime=C%d %s | rule=%s dir=%+d",
                     TimeToString(bt, TIME_DATE|TIME_MINUTES),
                     g_active_cluster, REGIME_NAMES[g_active_cluster],
   ...
   if(InpVerbose && vol_regime >= 0)
      PrintFormat("BTC: vol=%s (p=%.2f) lot_mult=%.2fx base=%.2f final=%.2f",
   ...
   if(dir == +1)
   {
      ...
      ok = g_trade.Buy(lots, _Symbol, tick.ask, sl, 0,
                       StringFormat("V7-L %s", RULE_NAMES[rule_id]));
   ...
   else
   {
      ...
      ok = g_trade.Sell(lots, _Symbol, tick.bid, sl, 0,
                        StringFormat("V7-S %s", RULE_NAMES[rule_id]));
      if(ok) { g_entry_price=tick.bid; g_entry_atr=atr; g_entry_dir=-1; g_bars_held=0; }
   }
   if(!ok && InpVerbose)
      PrintFormat("BTC: order failed (%d)", GetLastError());
}
//+------------------------------------------------------------------+
