"""
HERMES XAU FIX — edge_pullback + tt-trail on XAU **M15** bars.
...
double exposure on identical signals. This lab tests the SAME recipe on M15:
different timeframe -> different trades (true diversification) + bigger ATR ->
smaller spread-in-R (XAU's weak point).

Harness: identical protocol — live feature pipeline on M15 resampled bars, 6mo
test windows, 3y rolling train, train-only thr (~5/day target on M15), 1-slot cd3,
DEV 2020-07..2024-12 / HOLDOUT 2025-01+. Net at $0.10/0.20/0.30 per-trade /ATR.
Exit grid (dev-selected): base SL7/T2 maxh60 | tt after {6,15,30} M15 bars -> 0.75.
"""
...
TARGET_PER_DAY = 3.0; COOLDOWN = 3; DEV_END = pd.Timestamp("2025-01-01")
MAXH = 20   # 5h on M15 (20 bars) (same wall-clock as M1 maxh300)
...
s = m1.set_index("time")
df = pd.DataFrame({"open": s.open.resample("15min").first(), "high": s.high.resample("15min").max(),
...
log(f"M15 bars: {len(df):,} ({df.time.iloc[0].date()} -> {df.time.iloc[-1].date()})")
...
sig_atr = atr[idx]
log(f"candidates {len(idx):,} | median M15 ATR ${np.median(sig_atr):.3f} "
...
VAR = {"base SL7/T2 mh20": (0, 2.0), "tt5/0.75": (5, 0.75), "tt10/0.75": (10, 0.75), "tt15/0.75": (15, 0.75)}
SIMS = {k: sim_exit(idx, dirs, O, H, L, C, atr, n, 7.0, 2.0, MAXH, *v) for k, v in VAR.items()}
base_pnl = SIMS["base SL7/T2 mh20"][0]
log("sims done")
...
for dev, label in [(True, "DEV (selection)"), (False, "HOLDOUT (untouched)")]:
    print(f"\n{'='*92}\nXAU M15 edge_pullback lab — {label}\n{'='*92}\n{hdr}")
    for vname in VAR:
...
for vname in VAR:
    ...
    if tr:
        ...
        ser.to_csv(OUT / f"xau_m15_daily_{vname.split()[0].replace('/','_')}.csv")
json.dump(results, open(OUT / "xau_m15_results.json", "w"), default=str, indent=1)
log("xau m5 lab done")
