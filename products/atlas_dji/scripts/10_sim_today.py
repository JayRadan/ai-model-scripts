"""
10_sim_today.py — Atlas DJI live-equivalent simulation for the current trading day

WHAT THIS DOES
==============
Pulls fresh Dukascopy Dow Jones (US30) M1 bars for the last 3 days, runs Atlas's
STRICT-candle-reversal entry rule + the deployed Q model, and reports
every trade Atlas DJI would have opened today plus its outcome.
...
R_to_USD     = 0.75      # 0.01 lot US30 (broker-dependent)

BUNDLE_PATH = SERVER / "decision_engine/models/atlas_dji_validated.pkl"
...
print(f"[atlas_dji] Fetching Dow Jones M1 {start.isoformat()} → {end.isoformat()}")
...
print(f"\n=== Atlas DJI sim — {today_date} ===")
print(f"({len(today)} trades opened today)")
if not today:
    print("(no Atlas DJI trades fired today)")
    sys.exit(0)
...
# ── 7. Save equity PNG ─────────────────────────────────────────────────
out_dir = ROOT / "products/atlas_dji/scripts/_out"
out_dir.mkdir(parents=True, exist_ok=True)
fig, ax = plt.subplots(figsize=(13, 5))
eq_usd = eq * R_to_USD
eq_times = [t["entry_time"] for t in today]
ax.plot(eq_times, eq_usd, lw=1.8, color="#f97316", marker="o", markersize=4)
ax.axhline(0, color="k", lw=0.7)
ax.set_title(f"Atlas DJI sim {today_date} — {len(today)} trades, WR {wins / len(rs) * 100:.1f}%, "
