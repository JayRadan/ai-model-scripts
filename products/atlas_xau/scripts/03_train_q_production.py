...
def main():
    ...
    sup=streak_up(vel); sdn=streak_down(vel); df["vel_up_streak"]=sup
    ...
    print(f"  training XGBRegressor (production, full data, no cutoff) ...", flush=True)
    ...
    if qm.sum() > 0:
        ...
        wr=float((rs>0).mean())
        sumR=float(rs.sum())
        holdout_q1 = {"n":int(len(rs)),"wr":wr,"pf":pf,"sum_r":sumR}
        print(f"  HOLDOUT @ Q>={Q_THR} (per-candidate): n={len(rs):,} WR={wr*100:.1f}% PF={pf:.2f} sumR={sumR:+.0f}", flush=True)
    else: holdout_q1={"n":0}
