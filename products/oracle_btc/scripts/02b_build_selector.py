"""
BTC K=5 Rolling-Window Regime Selector
==========================================
...
Outputs the same files as the old 02_build_selector.py:
  - regime_fingerprints_btc_K5.csv  (now K5, but same filename for compat)
  - regime_selector_btc_K5.json     (now K5)
  - regime_clusters_btc_K5.png
...
# ── Load data ──
DATA_PATH = P.data("swing_v5_btc.csv")
...
fp_df.to_csv(P.data("regime_fingerprints_btc_K5.csv"), index=False)
print(f"\nSaved: regime_fingerprints_btc_K5.csv")
...
with open(P.data("regime_selector_btc_K5.json"), "w") as f:
    json.dump(selector, f, indent=2)
print(f"Saved: regime_selector_btc_K5.json")
...
ax.set_title("BTC K=5 Rolling-Window Clustering", color="#FFD700", fontsize=14)
ax.tick_params(colors="#5a7080")
for sp in ax.spines.values(): sp.set_edgecolor("#1e2a3a")
plt.savefig(P.data("regime_clusters_btc_K5.png"), dpi=140, bbox_inches="tight", facecolor="#080c12")
print(f"Saved: regime_clusters_btc_K5.png")
