"""
ATLAS DJI — time-boxed-patience trail test (strict-candle engine, DJI orderflow data).
...
Adapted for ATLAS DJI (same strict-candle engine, DJI orderflow data).
"""
...
t0 = time.time()
m1 = pd.read_parquet(ROOT / "data/m1_dji_orderflow.parquet").sort_values("time").reset_index(drop=True)
print(f"DJI {len(m1):,} bars  {m1.time.iloc[0]} -> {m1.time.iloc[-1]}", flush=True)
...
for dev, label in [(True, "DEV 2022-2024 (selection)"), (False, "HOLDOUT 2025+ (untouched)")]:
    print(f"\n{'='*74}\nATLAS DJI strict-candle, Q>=3.0, spread 0.30R — {label}\n{'='*74}")
...
json.dump(res, open(OUT / "dji_atlas_lab_results.json", "w"), default=str, indent=1)
