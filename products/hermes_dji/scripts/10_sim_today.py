"""
10_sim_today.py — Hermes DJI live-equivalent simulation for the current trading day

WHAT THIS DOES
==============
Pulls fresh Dukascopy Dow Jones (US30) M1 bars for the last 3 days, runs the SAME
feature pipeline + decision logic that the deployed server uses, and
reports every trade Hermes DJI would have opened today plus its outcome
...
DEPLOYED CONFIG MIRRORED (commercial/server/decision_engine/configs/hermes_dji.py)
=================================================================================
  symbol_base:    DJIUSD
  near_thr:       0.50    (pullback bars: |dist| ≤ 0.50 ATR)
  counter_thr:    1.5     (counter bars: dist_signed × cdir ≤ -1.5 ATR)
  q_thr:          3.0     (deployed 2026-05-27)
  dist_cap:       0.0     (not currently used on DJI)
  sl_hard_atr:    6.0
  trail_atr:      2.0
  be_trigger_r:   0.5
  max_hold_bars:  300
...
USAGE
=====
    python3 products/hermes_dji/scripts/10_sim_today.py

OUTPUT
======
  • Stdout table: every trade today with entry, exit, Q, dist, R, reason
  • Saved PNG: products/hermes_dji/scripts/_out/sim_today.png
"""
import sys, pickle
...
# ── DEPLOYED CONFIG — mirrors configs/hermes_dji.py exactly ────────────
SYMBOL_BASE  = "DJIUSD"
...
NEAR_THR     = 0.50
COUNTER_THR  = 1.5
Q_THR        = 3.0   # 2026-06-18 dynamic Q: strict chop default (raised 2.5→3.0)
Q_THR_TREND  = 1.0   # 2026-06-18 dynamic Q: looser threshold when trend_strong
TREND_AGE_MIN     = 30
TREND_SLOPE_MIN   = 1.0
TREND_DEMA50_MIN  = 1.0
DIST_CAP     = 0.0       # disabled on DJI
R_to_USD     = 0.75      # 0.01 lot US30 ≈ $0.75 / R (broker-dependent)

BUNDLE_PATH = SERVER / "decision_engine/models/hermes_dji_validated.pkl"
...
# ── 1. Fetch 3 days of US30 / Dow Jones M1 ──────────────────────────────────────
end = datetime.now(timezone.utc)
start = end - timedelta(days=3)
print(f"[hermes_dji] Fetching Dow Jones M1 {start.isoformat()} → {end.isoformat()}")
df = dukascopy_python.fetch(instrument=INSTRUMENT_IDX_AMERICA_E_D_J_IND, interval=dukascopy_python.INTERVAL_MIN_1,
...
print(f"\n=== Hermes DJI sim — {today_date} ===")
print(f"({len(today)} trades opened today)")
if not today:
    print("(no Hermes DJI trades fired today)")
    sys.exit(0)
...
# ── 7. Save equity PNG ─────────────────────────────────────────────────
out_dir = ROOT / "products/hermes_dji/scripts/_out"
out_dir.mkdir(parents=True, exist_ok=True)
fig, ax = plt.subplots(figsize=(13, 5))
eq_usd = eq * R_to_USD
eq_times = [t["entry_time"] for t in today]
ax.plot(eq_times, eq_usd, lw=1.8, color="#ef4444", marker="o", markersize=4)
ax.axhline(0, color="k", lw=0.7)
ax.set_title(f"Hermes DJI sim {today_date} — {len(today)} trades, WR {wins / len(rs) * 100:.1f}%, "
