"""Sweep filter variants on 30-day unseen data.

For each product test:
  - Time filter: different block windows (UTC hours)
  - Trend gate: different slope thresholds

Goal: find the *best* variant per product, not just verify the rough idea works.
"""
...
def hermes_signals(df, bundle_pkl):
    tfk_df=compute_tfk(df); fdf=add_lookahead_htf(df)
    ...
    return {"idxs":idxs_all,"dirs":dirs_all,"q":q_all,"is_counter":is_ct[idxs_all],"is_pullback":is_pb[idxs_all],
            ...
            "atr":atr,"n":n,"sp":sp,"O":O,"H":H,"L":L,"C":C,"df":df,"times":df["time"].to_numpy()}
...
def filter_signals(data, q_thr, dist_cap=None, time_block=None, trend_slope=None):
    """time_block = (start_hour, end_hour) UTC, e.g. (20, 1) means block 20-01.
       trend_slope = threshold to block counter-trend entries."""
    idxs=data["idxs"].copy(); dirs=data["dirs"].copy(); q=data["q"].copy()
    is_ct=data["is_counter"]; is_pb=data["is_pullback"]; dist_abs=data["dist_abs"]
    slope20=data["slope20"]; times=data["times"]
    keep = q >= q_thr
    if dist_cap is not None:
        stretched = is_ct & ~is_pb & (dist_abs > dist_cap); keep = keep & ~stretched
    ...
    if trend_slope is not None:
        slope_sign = np.sign(slope20); d_sign = dirs.astype(float)
        counter_trend = (d_sign * slope_sign) < 0
        if is_ct.any():
            block = is_ct & counter_trend & (np.abs(slope20) > trend_slope)
        else:
            block = counter_trend & (np.abs(slope20) > trend_slope)
        keep = keep & ~block
...
def stats_short(executed, label="", rate=1.5):
    if not executed: return f"  {label:30} | (0 trades)"
    ...
    d10 = rs.sum()*rate*10
    pfs = f"{pf:5.2f}" if pf<1000 else "  inf"
    return f"  {label:30} | trd={len(rs):4} WR={w/len(rs)*100:5.1f}% PF={pfs:>6} sumR={rs.sum():+8.2f} DD={dd:6.2f} ${d10:+10.2f}@.10"


# Fetch + signal cache
print("Fetching 30 days XAU + BTC + DJI ...")
xau = fetch("XAU/USD", days=30); btc = fetch("BTC/USD", days=30); dji = fetch("XAU/USD", days=30, dji=True)
...
print("\n" + "="*120)
print(f"  TIME-FILTER SWEEP — block UTC hours [start..end)")
print("="*120)
for prod_name in ["hermes_xau","hermes_btc","hermes_dji","atlas_xau","atlas_btc","atlas_dji"]:
    data, qthr, dist_cap, rate = PROD_SIGS[prod_name]
    print(f"\n──── {prod_name} ────")
    for tv in TIME_VARIANTS:
        lbl = "baseline" if tv is None else f"block {tv[0]:02d}-{tv[1]:02d}"
        idxs,dirs,q = filter_signals(data, qthr, dist_cap=dist_cap, time_block=tv)
        print(stats_short(sim(data, idxs, dirs, q), label=lbl, rate=rate))

# TREND-GATE SWEEP — only on the products where it might help
print("\n" + "="*120)
print(f"  TREND-GATE SWEEP — block counter when |slope20| > threshold AND direction opposite slope")
print("="*120)
SLOPE_VARIANTS = [None, 1.0, 1.5, 2.0, 2.5, 3.0, 3.5, 4.0, 5.0]
for prod_name in ["hermes_xau","hermes_btc","hermes_dji","atlas_xau","atlas_btc","atlas_dji"]:
    data, qthr, dist_cap, rate = PROD_SIGS[prod_name]
    print(f"\n──── {prod_name} ────")
    for sv in SLOPE_VARIANTS:
        lbl = "baseline" if sv is None else f"slope>{sv}"
        idxs,dirs,q = filter_signals(data, qthr, dist_cap=dist_cap, trend_slope=sv)
        print(stats_short(sim(data, idxs, dirs, q), label=lbl, rate=rate))
