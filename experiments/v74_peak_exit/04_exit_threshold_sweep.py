"""
v7.4 Peak-Exit experiment — sweep the ML exit threshold.

Production EXIT_THRESHOLD = 0.55. Lower = exit head fires more aggressively
(closes trades earlier). Higher = exit head only fires on near-certain peaks.

Compares thresholds {0.30 .. 0.70} on the same Janus holdout, max_hold=60,
production exit head (no retrain).
"""
...
def simulate(confirmed, swing, atr, exit_mdl, exit_threshold, max_hold=60):
...
def main():
    ...
    print(f"  meta-filtered fold: {len(fcm):,} setups\n", flush=True)

    print(f"{'thr':>5s}  {'n':>5s} {'WR':>6s} {'PF':>6s} {'+R':>9s} {'DD':>7s} {'avg_bars':>9s}  {'ml_exit%':>9s}  {'sl%':>5s}  {'maxh%':>5s}")
    print("-" * 88)
    rows = []
    for thr in THRESHOLDS:
        t = simulate(fcm, swing, atr, payload["exit_mdl"], thr, max_hold=60)
        r = report(t)
        if not r: continue
        ed = r["exit_dist"]
        ml_pct = ed.get("ml_exit", 0) / r["n"] * 100
        sl_pct = ed.get("sl", 0) / r["n"] * 100
        mh_pct = ed.get("max_hold", 0) / r["n"] * 100
        print(f"{thr:>5.2f}  {r['n']:>5d} {r['wr']:>5.1%} {r['pf']:>6.2f} "
              f"{r['total_R']:>+9.0f} {r['dd']:>7.0f} {r['avg_bars']:>9.1f}  "
              f"{ml_pct:>8.1f}%  {sl_pct:>4.1f}%  {mh_pct:>4.1f}%")
        rows.append({"thr": thr, **r})

    with open(os.path.join(OUT_DIR, "reports", "exit_threshold_sweep.json"), "w") as f:
        json.dump(rows, f, indent=2, default=str)
    print(f"\nSaved {OUT_DIR}/reports/exit_threshold_sweep.json")
