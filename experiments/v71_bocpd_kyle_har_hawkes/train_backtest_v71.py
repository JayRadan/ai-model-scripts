"""
CLEAN v6 vs v7.1 — global time cutoff + ML exit.

v7.1 = v6 (14) + vpin + sig_quad_var + bocpd_recent_cp + kyle_lambda
       + har_rv_ratio + hawkes_eta  = 20 features total.
"""
...
NEW_FEATS = ["vpin", "sig_quad_var", "bocpd_recent_cp",
             "kyle_lambda", "har_rv_ratio", "hawkes_eta"]
ALL_FEATS = OLD_FEATS + NEW_FEATS     # 20
...
def main():
    print("=" * 78)
    print("CLEAN v6 vs v7.1 — VPIN + quad_var + BOCPD + Kyle's λ + HAR-RV + Hawkes")
    ...
    # ---------- v7.1 (20 feats) ----------
    print("\nTraining v7.1 (20 features = 14 + VPIN + quad_var + BOCPD + Kyle + HAR + Hawkes)...")
    t0 = _time.time()
    v71_mdls, v71_thrs = train_confirmation_models(train, ALL_FEATS, "v7.1")
    print(f"  conf train {_time.time()-t0:.0f}s")
    t0 = _time.time()
    v71_train_conf = confirm_setups(train, v71_mdls, v71_thrs, ALL_FEATS)
    print(f"  v7.1: {len(v71_train_conf):,} confirmed train trades")
    v71_exit_mdl = train_exit_model(v71_train_conf, swing, atr, "v7.1")
    print(f"  exit train {_time.time()-t0:.0f}s")
    ...
    v71_conf_test = confirm_setups(test, v71_mdls, v71_thrs, ALL_FEATS)
    print(f"  v6 confirmed in holdout:   {len(v6_conf_test):,}")
    print(f"  v7.1 confirmed in holdout: {len(v71_conf_test):,}")

    v6_res = simulate_with_ml_exit(v6_conf_test, swing, atr, v6_exit_mdl, "v6")
    v71_res = simulate_with_ml_exit(v71_conf_test, swing, atr, v71_exit_mdl, "v7.1")

    if v6_res and v71_res:
        print(f"\n{'='*78}")
        print("COMPARISON — clean holdout, ML exit")
        print(f"{'='*78}")
        print(f"{'Metric':<18} {'v6 (14)':>13} {'v7.1 (20)':>13} {'Delta':>11}")
        print(f"{'Trades':<18} {v6_res['n']:>13} {v71_res['n']:>13} {v71_res['n']-v6_res['n']:>+11}")
        print(f"{'Win rate':<18} {v6_res['wr']*100:>12.1f}% {v71_res['wr']*100:>12.1f}% {(v71_res['wr']-v6_res['wr'])*100:>+10.1f}pt")
        print(f"{'Profit factor':<18} {v6_res['pf']:>13.2f} {v71_res['pf']:>13.2f} {v71_res['pf']-v6_res['pf']:>+11.2f}")
        print(f"{'Expectancy R':<18} {v6_res['expectancy']:>+13.3f} {v71_res['expectancy']:>+13.3f} {v71_res['expectancy']-v6_res['expectancy']:>+11.3f}")
        print(f"{'Total PnL R':<18} {v6_res['total_R']:>+13.1f} {v71_res['total_R']:>+13.1f} {v71_res['total_R']-v6_res['total_R']:>+11.1f}")
        print(f"{'Max DD R':<18} {-v6_res['max_dd']:>+13.1f} {-v71_res['max_dd']:>+13.1f} {v6_res['max_dd']-v71_res['max_dd']:>+11.1f}")

        out_dir = "/home/jay/Desktop/new-model-zigzag/experiments/v71_bocpd_kyle_har_hawkes"
        v6_res["trades"].to_csv(f"{out_dir}/v6_trades_holdout.csv", index=False)
        v71_res["trades"].to_csv(f"{out_dir}/v71_trades_holdout.csv", index=False)
        ...
        try:
            ...
            eq71 = v71_res["trades"]["pnl_R"].cumsum().values
            t6 = pd.to_datetime(v6_res["trades"]["time"])
            t71 = pd.to_datetime(v71_res["trades"]["time"])
            ax.plot(t6, eq6, label=f"v6 (14)  PF={v6_res['pf']:.2f}  n={v6_res['n']}", lw=1.8)
            ax.plot(t71, eq71, label=f"v7.1 (20)  PF={v71_res['pf']:.2f}  n={v71_res['n']}", lw=1.8)
            ax.axhline(0, color="k", lw=0.5)
            ax.set_ylabel("Cumulative R")
            ax.set_title("v6 vs v7.1 — clean holdout, ML exit")
            ax.legend(loc="best")
            ax.grid(alpha=0.3)
            fig.tight_layout()
            fig.savefig(f"{out_dir}/v6_vs_v71_holdout.png", dpi=110)
            print(f"  Saved: {out_dir}/v6_vs_v71_holdout.png")
