"""
v7.3 Pivot Oracle validation — IDENTICAL to v7.2-lite Oracle pipeline,
only difference: setups come from RP_* turning-point detectors instead of
the 28 chart-pattern rules. Same 18 v72L features, same regime clusters,
same exit head, same meta gate.

If holdout PF stays competitive vs v7.2-lite Oracle (3.96), the pivot rules
are a viable replacement / addition. If not, pivot-only doesn't carry an edge.
"""
...
def load_and_split():
    all_setups = []
    V73_DATA = "/home/jay/Desktop/new-model-zigzag/experiments/v73_pivot_oracle/data"
    for f in sorted(glob.glob(os.path.join(V73_DATA, "setups_*_v73p.csv"))):
...
def main():
    ...
    out_trades = "/home/jay/Desktop/new-model-zigzag/experiments/v73_pivot_oracle/reports/v73_trades_holdout.csv"
    ...
    with open("/home/jay/Desktop/new-model-zigzag/experiments/v73_pivot_oracle/reports/meta_threshold_v73.txt", "w") as f:
        f.write(f"{best_thr:.4f}\n")
    print(f"\n  Saved meta threshold to meta_threshold_v72l.txt: {best_thr:.4f}")
