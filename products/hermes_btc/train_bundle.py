"""
Hermes BTC — train + freeze the production Q-regressor bundle.

Trains XGBRegressor on the v103 spec:
  - Source bars: data/m1_xau_full.parquet (17 mo Dukascopy M1) JOINED with
                 data/m1_btc_orderflow.parquet for the tick-derived features
  - Train cutoff: 2026-01-15 (research holdout split)
  - Label: forward pnl_R with SL=4×ATR, trail=3×ATR, max_hold=300, spread-adjusted
  - Features: 29 standard + 14 order-flow = 43 total
  - Output: commercial/server/decision_engine/models/hermes_btc_validated.pkl
...
# ── Config (must match commercial/server/.../configs/hermes_btc.py) ─────
CUTOFF = pd.Timestamp("2026-01-15 00:00:00")
SL = 4.0
TRAIL = 3.0
MAX_HOLD = 300
NEAR = 0.50         # candidate gate (training uses tighter so we get clean labels)
                    # (Live decide uses the same near_thr)
BTC_SPREAD_USD = 5.0

OUT_DIR = Path("/home/jay/Desktop/my-agents-and-website/commercial/server/"
               "decision_engine/models")
OUT_PKL = OUT_DIR / "hermes_btc_validated.pkl"
...
def main():
    print("="*72)
    print("  Hermes BTC — bundle training pipeline (M1 + order-flow)")
    print("="*72)
    t0 = time.time()
    ...
    # Simple holdout sanity: PF at Q>=1.0
    mask = q_te >= 1.0
    bare = metrics(pnl_s[test_m][mask])
    print(f"  holdout @ Q>=1.0: {bare}", flush=True)
    ...
    payload = {
        ...
        "train_meta": {
            ...
            "holdout_q1": bare,
            "config_snapshot": {
                "NEAR": NEAR, "SL": SL, "TRAIL": TRAIL, "MAX_HOLD": MAX_HOLD,
                "SPREAD_USD": BTC_SPREAD_USD,
            },
        },
    }
