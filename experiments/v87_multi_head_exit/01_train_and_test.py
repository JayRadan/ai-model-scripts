"""v87: Multi-Head Exit Model — Full Training + Policy Sweep

Trains 4 XGBClassifier heads that predict:
...
Then sweeps policy thresholds to find optimal exit rules.
Compares against current binary ML exit.
"""
...
print("="*60)
print("  v87 Multi-Head Exit Model")
print("="*60)

# ═══ 1. Load data ═══
print("[1/5] Loading...", end='', flush=True); t0=_time.time()
...
trades=pd.read_csv(f"{PROJECT}/experiments/v84_rl_entry/v84_rl_trades.csv",parse_dates=["time"])
...
# We need to map tst_ed rows back to trades
# Build a trade_id per sample
trade_ids=[]
...
def apply_policy(probas_dict, trade_ids_arr, thresholds, test_trades_df):
    ...
    for ti in range(n_trades):
        ...
        exit_bar=None; exit_reason="max"
        # Find samples for this trade
        sample_idxs=[j for j,tid in enumerate(trade_ids_arr) if tid==ti]
        
        for j in sample_idxs:
            if j>=len(probas_dict['label_up']): break
            p_up=probas_dict['label_up'][j]
            p_gb=probas_dict['label_gb'][j]
            p_sl=probas_dict['label_sl'][j]
            p_nh=probas_dict['label_nh'][j]
            # We don't have bar-level mtm easily from here
            # Simplified: combine probabilities into decision
            pass  # need bar-level simulation, not sample-level
        ...
        for k in range(1,MAX_HOLD+1):
            ...
            if bar>=n: break
            mtm=d*(C[bar]-ep)/ea
            if mtm<=SL_HARD: exit_bar=bar; exit_reason="hard_sl"; break
            ...
            if exit_signal:
                exit_bar=bar; exit_reason="multi_head"; break
...
bundle={
    ...
    'version':'v87-multi-head-exit',
}
with open(f"{OUT}/multi_head_exit_bundle.pkl",'wb') as f:
    pickle.dump(bundle,f)
print(f" ✓ → {OUT}/multi_head_exit_bundle.pkl",flush=True)

print(f"\n{'='*60}")
print(f"  Multi-Head Exit Model — Summary")
