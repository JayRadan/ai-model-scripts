"""
Aggregate Dukascopy ticks → M1 with ORDER-FLOW features.

For each M1 bar, compute:
  signed_flow         — sum of signed_volume (price up = +vol, down = -vol)
  imbalance_ratio     — signed_flow / abs_volume   (∈ [-1, 1])
  n_ticks             — number of tick events
  bid_ask_vol_ratio   — sum(bidVol) / (sum(bidVol)+sum(askVol)) at quote level
  median_spread       — typical bid-ask spread
  vpin_proxy          — |signed_flow| / abs_volume (within-bar VPIN approx)
  cum_signed_5        — sum of signed_flow over last 5 M1 bars
  cum_signed_15       — sum of signed_flow over last 15 M1 bars
  cum_signed_60       — sum of signed_flow over last 60 M1 bars (1 hour)

Saves to data/m1_xau_orderflow.parquet.
"""
...
OUT = ROOT / "data" / "m1_xau_orderflow.parquet"
...
def aggregate_one_day(parquet_path: str) -> pd.DataFrame:
    ...
    tot_v = bv + av
    # tick rule: signed_volume based on mid change vs previous tick
...
def main():
    ...
    tick_dir = ROOT / "data" / "ticks" / "xau"
    parquets = sorted(glob(str(tick_dir / "*.parquet")))
    print(f"  aggregating {len(parquets)} tick parquets → M1 with orderflow ...", flush=True)
    t0 = time.time()
    frames = []
