# atlas_xau — pipeline scripts

End-to-end scripts for the **atlas_xau** product, in execution order.
...
| 3 | `03_train_q_production.py` | Train the XGBRegressor Q-model on MFE≥2R candidates + write the .pkl bundle | `commercial/server/decision_engine/models/atlas_xau_validated.pkl` |
| 10 | `10_sim_today.py` | Simulate today's trades under the **deployed** config (Q≥2.0, time_block_utc=(18, 2)). Outputs trade-by-trade table + equity PNG | `scripts/_out/sim_today.png` |

## Deployed config (as of 2026-06-09)

| Parameter | Value | Source of truth |
|---|---|---|
| Q threshold | **2.0** | `commercial/server/decision_engine/configs/atlas_xau.py` |
| Filter | **time_block_utc=(18, 2)** | Same config |
...
python3 products/atlas_xau/scripts/01_download_m1_dukascopy.py

# Step 2 — aggregate tick stream into order-flow features (~5 min)
python3 products/atlas_xau/scripts/02_aggregate_orderflow_from_ticks.py

# Step 3 — train and write the production bundle (~1-3 min)
python3 products/atlas_xau/scripts/03_train_q_production.py
```

After step 3, the new `atlas_xau_validated.pkl` is in the server's models/ directory.
**Commit it via the commercial repo** to deploy.

## How to verify a config change without redeploying

```bash
# Edit the config in commercial/server/decision_engine/configs/atlas_xau.py
...
in sync with `commercial/server/decision_engine/configs/atlas_xau.py` whenever
