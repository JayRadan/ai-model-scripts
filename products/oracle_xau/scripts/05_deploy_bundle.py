"""Deploy RL Entry models to server."""
...
print("="*60)
print("  Deploying RL Entry to server...")
print("="*60)
...
# ═══ Train RL Entry ═══
print("[2/4] Training RL Entry Q-models...", flush=True); t0=_time.time()
q_entry={}
for cid in range(5):
    ...
    print(f"  C{cid}: {pos:,} bars with Q>{MIN_Q}R ({pos/len(g):.1%})", flush=True)
...
rl_train=gen(train_setups)
print(f"  {len(rl_train):,} RL entries, {_time.time()-t0:.0f}s", flush=True)
...
# ═══ Save & Deploy ═══
print("[4/4] Saving + deploying...", flush=True)

bundle = {
    "q_entry": q_entry,  # 5 XGBRegressor dict
    "mdls": c_mdls, "thrs": c_thrs,
    "exit_mdl": exit_mdl,
    "meta_mdl": meta_mdl, "meta_threshold": float(best_thr),
    "v72l_feats": V72L_FEATS, "meta_feats": META_FEATS, "exit_feats": EXIT_FEATS,
    "min_q": MIN_Q,
    "rl_rule_name": "RL",
    "trained_on": f"v84 RL Entry — Q-Learning per regime (PF 4.21 on holdout, {len(q_entry)} regimes)",
    "version": "v84-rl",
}

with open(f"{SERVER}/models/oracle_xau_validated.pkl", "rb") as f:
    old_bundle = pickle.load(f)

# Keep old confirm/exit/meta by default, but add RL entry models
# Actually, replace the entire bundle since RL entry needs its own confirm models
with open(f"{SERVER}/models/oracle_xau_validated.pkl", "wb") as f:
    pickle.dump(bundle, f)

# Save backup locally
with open(f"{OUT}/rl_deploy_bundle.pkl", "wb") as f:
    pickle.dump(bundle, f)

print(f"  Deployed: {SERVER}/models/oracle_xau_validated.pkl")
print(f"  Entry models: {len(q_entry)} Q-functions")
print(f"  Confirm models: {len(c_mdls)}")
