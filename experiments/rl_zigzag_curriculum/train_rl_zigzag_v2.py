...
PHASE_STEPS = 1_200_000; N_ENVS = 16; SEED = 0
REHEARSAL = 0.30
...
def train_variant(name, rehearsal, joint=False):
    ...
    model = PPO("MlpPolicy", venv, learning_rate=3e-4, n_steps=256, batch_size=1024,
                gamma=0.997, gae_lambda=0.95, ent_coef=0.005, seed=SEED, verbose=0,
    ...
    for pi_, (pname, ci) in enumerate(phases):
        ...
        for env in venv.envs: env.set_shape(0.3, 0.0 if first_phase else 2.0)
...
RES = {}; FM = {}
for name, reh, joint in [("REH", True, False), ("JOINT", False, True)]:
...
plt.tight_layout(); plt.savefig(OUT / "rl_zigzag_v2_holdout.png", dpi=110)
json.dump({vn: {k: {kk: vv for kk, vv in r.items() if kk != "eq"} for k, r in RES[vn].items()} for vn in RES},
          open(OUT / "rl_zigzag_v2_results.json", "w"), indent=1)
log(f"done -> {OUT/'rl_zigzag_v2_holdout.png'}")
