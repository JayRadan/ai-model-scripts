...
def main():
    ...
    trades = pd.read_csv(HERE / "trade_charts" / "btc_m1_full_q1.0_trades.csv",
                         parse_dates=["time"])
    ...
    a0.set_title(f"BTC M1 FULL — Q≥1.0 with $0.30 spread — n={len(trades)} "
    ...
    plt.tight_layout()
    plt.savefig(out_dir / "btc_m1_full_equity.png", dpi=110)
    plt.close()
    print(f"  wrote btc_m1_full_equity.png", flush=True)
    ...
    for w_idx, center in enumerate(selected_centers):
        ...
        ax.set_title(f"BTC M1 — cluster {w_idx+1}/4 — {local_times[0]} to {local_times[-1]}\n"
                     f"n={len(in_win)} wins={len(ww)} losses={len(ll)} sumR={in_win.pnl_R.sum():+.1f}R",
                     fontsize=11)
        ax.set_ylabel("price"); ax.grid(alpha=0.2)
        plt.tight_layout()
        plt.savefig(out_dir / f"btc_m1_full_cluster_{w_idx+1}.png", dpi=110)
        plt.close()
