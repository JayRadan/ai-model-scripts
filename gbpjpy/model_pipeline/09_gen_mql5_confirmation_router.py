"""
Generate confirmation_router_gbpjpy.mqh — embeds the per-rule probability
thresholds as constants and exposes a single ConfirmRule() function.
GBPJPY version — 48 rules. FEATURE_DIM = 34 (21 base + 13 tech).
"""
import json
import os
...
for rule_name, cid in RULES:
    ...
    onnx_files.append(f"gj_confirm_c{cid}_{rule_name}.onnx")

lines = []
lines.append("//+------------------------------------------------------------------+")
lines.append("//| confirmation_router_gbpjpy.mqh                                   |")
lines.append(f"//| Auto-generated. Loads {len(RULES)} per-rule ONNX classifiers.              |")
lines.append("//+------------------------------------------------------------------+")
lines.append("#ifndef __CONFIRMATION_ROUTER_GBPJPY_MQH__")
lines.append("#define __CONFIRMATION_ROUTER_GBPJPY_MQH__")
lines.append("")
lines.append('#include "setup_rules_gbpjpy.mqh"')
lines.append("")
...
lines.append("}")
lines.append("")
lines.append("#endif  // __CONFIRMATION_ROUTER_GBPJPY_MQH__")

text = "\n".join(lines) + "\n"
out = str(P.LIVE_DIR / "MQL5_Include" / "confirmation_router_gbpjpy.mqh")
